...
import com.yorel.muxon.grpc.workflow.v1.MigrateVMRequest;
...
import com.yorel.muxon.grpc.workflow.v1.ResolveConsoleRequest;
...
/**
 * gRPC implementation of {@link VMWorkflowServiceGrpc.VMWorkflowServiceImplBase}. Each method
 * creates a Job + enqueues a task via {@link JobService} and returns a {@link JobResponse}.
 */
@Service
    ...
    handleRequest(
        ...
        () -> {
          ...
          Map<String, Object> payload =
              simpleVmPayload(request.getExternalId(), request.getGrantId());
          return jobService.createJob(
              JobType.VM_DELETE,
              EntityType.VM,
              UUID.fromString(request.getVmId()),
  ...
  public void attachIso(AttachIsoVMRequest request, StreamObserver<JobResponse> responseObserver) {
    handleRequest(
        ...
        () -> {
          ...
          return jobService.createJob(
              JobType.VM_CREATE,
              EntityType.VM, // reuse nearest type; extend JobType if needed
              UUID.fromString(request.getVmId()),
              request.getCorrelationId(),
              "VM_ATTACH_ISO_COMMAND",
              payload);
        });
  }
    ...
    handleRequest(
        ...
        () -> {
          ...
          return jobService.createJob(
              ...
              UUID.fromString(request.getVmId()),
              request.getCorrelationId(),
              "VM_DETACH_ISO_COMMAND",
              payload);
        });
  }

  @Override
  public void publishVMTemplate(
      PublishVMTemplateRequest request, StreamObserver<JobResponse> responseObserver) {
    handleRequest(
        responseObserver,
        () -> {
          log.debug(
          ...
          return jobService.createJob(
              ...
              UUID.fromString(request.getVmId()),
              request.getCorrelationId(),
              "VM_PUBLISH_TEMPLATE_COMMAND",
              payload);
        });
  }

  @Override
  public void resolveConsole(
      ResolveConsoleRequest request, StreamObserver<JobResponse> responseObserver) {
    handleRequest(
        responseObserver,
        () -> {
          log.debug(
          ...
          return jobService.createJob(
              ...
              UUID.fromString(request.getVmId()),
              request.getCorrelationId(),
              VmQueueCommands.CONSOLE_RESOLVE,
              payload);
        });
  }
