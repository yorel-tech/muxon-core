...
/**
 * Core orchestrator service. Creates a Job, decomposes it into tasks, enqueues tasks for worker
 * pickup, and returns the Job synchronously so callers can immediately return a job reference to
 * the end user.
 *
...
public class JobService {
  ...
  /**
   * Create a job for the given workflow, enqueue its task(s) to the CommandQueue, and return a
   * gRPC-ready {@link JobResponse}.
   *
  ...
  public JobResponse createJob(
    ...
    Map<String, Object> enriched =
        new java.util.HashMap<>(taskPayload != null ? taskPayload : Map.of());
    ...
    UUID taskId =
        commandQueue.sendCommand(
            CommandMessage.builder()
                .queueType(queueType)
                .entityType(com.yorel.muxon.api.model.EntityType.valueOf(entityType.name()))
                .entityId(entityId)
                .payload(enriched)
  ...
  public org.springframework.data.domain.Page<JobEntity> listJobs(
    ...
    if (entityType != null && entityId != null && status != null) {
      ...
      return jobRepository.findByTargetEntityTypeAndTargetEntityId(entityType, entityId, pageable);
    } else if (status != null) {
  ...
  /**
   * Safe, compact description of task payload for DEBUG logs (no credentials; large JSON as length
   * only).
   */
