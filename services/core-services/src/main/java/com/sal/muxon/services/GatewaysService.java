...
import com.sal.muxon.db.model.FloatingIpEntity.FloatingIpStatus;
import com.sal.muxon.db.model.InternetGatewayEntity;
...
import com.sal.muxon.db.repository.InternetGatewayRepository;
import com.sal.muxon.db.repository.NatGatewayRepository;
...
public class GatewaysService {
    ...
    @Autowired
    private InternetGatewayRepository igwRepository;

    @Autowired
    private NatGatewayRepository natRepository;
    ...
    @Autowired
    private PublicIpPoolsService poolsService;
    ...
    public InternetGatewayEntity createInternetGateway(UUID vpcId, String name) {
        ...
        // Find active edge node in the VPC's datacenter (subnets carry datacenter, use first subnet's datacenter)
        List<SubnetEntity> subnets = subnetRepository.findByVpcId(vpcId);
        UUID datacenterId = subnets.isEmpty() ? null :
                subnets.get(0).getDatacenter().getId();

        NetworkEdgeNodeEntity edgeNode = null;
        if (datacenterId != null) {
            edgeNode = edgeNodesService.findActiveEdgeNode(datacenterId)
                    .orElseThrow(() -> new IllegalStateException(
                            "UNPROCESSABLE: No active network edge node available in datacenter " + datacenterId));
        }
    ...
    public void deleteInternetGateway(UUID vpcId) {
        InternetGatewayEntity igw = igwRepository.findByVpcId(vpcId)
                .orElseThrow(() -> new EntityNotFoundException("Internet gateway not found for VPC: " + vpcId));
    ...
    @Transactional
    public NatGatewayEntity createNatGateway(UUID vpcId, String name, UUID subnetId, UUID floatingIpId) {
        VpcEntity vpc = vpcRepository.findById(vpcId)
                .orElseThrow(() -> new EntityNotFoundException("VPC not found: " + vpcId));
        ...
        if (!subnet.isPublic()) {
            throw new IllegalArgumentException("UNPROCESSABLE: NAT gateway must be placed in a public subnet");
        }

        FloatingIpEntity floatingIp = floatingIpRepository.findById(floatingIpId)
                .orElseThrow(() -> new EntityNotFoundException("Floating IP not found: " + floatingIpId));

        NetworkEdgeNodeEntity edgeNode = edgeNodesService.findActiveEdgeNode(subnet.getDatacenter().getId())
                .orElse(null);
    ...
    public NatGatewayEntity getNatGateway(UUID natGatewayId) {
        return natRepository.findById(natGatewayId)
    ...
    public FloatingIpEntity allocateFloatingIp(UUID tenantId, UUID datacenterId) {
        ...
        PublicIpPoolEntity pool = poolsService.listByDatacenter(datacenterId).stream()
                .filter(p -> "ACTIVE".equals(p.getStatus()) && p.getAllocatedIps() < p.getTotalIps())
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("POOL_EXHAUSTED: No available public IPs in datacenter " + datacenterId));
    ...
    public FloatingIpEntity getFloatingIp(UUID floatingIpId) {
        return floatingIpRepository.findById(floatingIpId)
                .orElseThrow(() -> new EntityNotFoundException("Floating IP not found: " + floatingIpId));
    }
