package com.sal.muxon.services.task;

import com.sal.muxon.api.dto.*;
import com.sal.muxon.api.enums.EntityType;
import com.sal.muxon.api.enums.JobStatus;
import com.sal.muxon.api.enums.JobType;
import com.sal.muxon.db.model.JobEntity;
import com.sal.muxon.db.model.TaskLogEntity;
import com.sal.muxon.db.model.TaskStepEntity;
import com.sal.muxon.db.model.VmEntity;
import com.sal.muxon.db.repository.JobRepository;
import com.sal.muxon.db.repository.TaskLogRepository;
import com.sal.muxon.db.repository.TaskStepRepository;
import com.sal.muxon.db.repository.VmRepository;
import com.sal.muxon.services.statemachine.EntityStateMachine;
...
public class TaskOrchestrationService {
    ...
    public JobEntity startTask(UUID taskId) {
        JobEntity task = jobRepository.findById(taskId)
    ...
    @Transactional
    public TaskStepEntity createStep(UUID taskId, int stepNumber, String stepName, String stepDescription) {
        JobEntity task = jobRepository.findById(taskId)
                .orElseThrow(() -> new IllegalArgumentException("Task not found: " + taskId));
    ...
    public TaskStepEntity startStep(UUID taskId, int stepNumber) {
        TaskStepEntity step = taskStepRepository.findByTaskIdAndStepNumber(taskId, stepNumber)
        ...
        logTaskEvent(taskId, step.getId(), "INFO", "Step " + stepNumber + " started: " + step.getStepName());
    ...
    public TaskStepEntity completeStep(UUID taskId, int stepNumber, Object output) {
        ...
        logTaskEvent(taskId, step.getId(), "INFO", "Step " + stepNumber + " completed: " + step.getStepName());
    ...
    public JobEntity completeTask(UUID taskId, Object result) {
        ...
        entityStateMachine.updateStateForTaskStatus(task.getTargetEntityId(), task, JobStatus.COMPLETED);
    ...
    public JobEntity cancelTask(UUID taskId, String reason) {
        ...
        if (task.getStatus() == JobStatus.PENDING) {
            ...
            task.setCompletedAt(Instant.now());
            entityStateMachine.updateStateForTaskStatus(task.getTargetEntityId(), task, JobStatus.CANCELLED);
        }
    ...
    public Page<TaskResponse> listTasks(EntityType entityType, UUID entityId, JobStatus status, JobType operation, Pageable pageable) {
        Page<JobEntity> tasks;

        if (entityType != null && entityId != null) {
            if (status != null) {
                tasks = jobRepository.findByTargetEntityTypeAndTargetEntityIdAndStatus(entityType, entityId, status, pageable);
            } else {
                tasks = jobRepository.findByTargetEntityTypeAndTargetEntityId(entityType, entityId, pageable);
            }
        } else if (status != null) {
    ...
    private TaskResponse buildTaskResponse(JobEntity task, boolean includeSteps, boolean includeLogs) {
        ...
        if (includeSteps) {
            List<TaskStepEntity> steps = taskStepRepository.findByTaskIdOrderByStepNumberAsc(task.getId());
            Optional<TaskStepEntity> runningStep = steps.stream()
                    .filter(s -> s.getStatus() == JobStatus.RUNNING)
                    .findFirst();
        ...
        if (task.getStartedAt() != null && task.getCompletedAt() != null) {
            timing.setDurationSeconds(Duration.between(task.getStartedAt(), task.getCompletedAt()).toMillis() / 1000.0);
        } else if (task.getStartedAt() != null) {
            timing.setDurationSeconds(Duration.between(task.getStartedAt(), Instant.now()).toMillis() / 1000.0);
        }
        ...
        if (includeLogs) {
            Page<TaskLogEntity> logsPage = taskLogRepository.findByTaskIdOrderByCreatedAtDesc(task.getId(), PageRequest.of(0, 100));
            response.setLogs(logsPage.getContent().stream().map(this::buildLogResponse).collect(Collectors.toList()));
        }
    ...
    private TaskStepResponse buildStepResponse(TaskStepEntity step) {
        ...
        if (step.getStartedAt() != null && step.getCompletedAt() != null) {
            response.setDurationSeconds(Duration.between(step.getStartedAt(), step.getCompletedAt()).toMillis() / 1000.0);
        } else if (step.getStartedAt() != null) {
            response.setDurationSeconds(Duration.between(step.getStartedAt(), Instant.now()).toMillis() / 1000.0);
        }
