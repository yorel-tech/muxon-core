...
import com.yorel.muxon.db.repository.VolumeAttachmentRepository;
 ...
 *
 * <p>Handles volume creation, deletion, resizing, and attachment/detachment to resources (VMs,
 * pods, containers). Validates storage class support and enforces business rules such as preventing
 * deletion of attached volumes.
 *
 * <p>Volume lifecycle: creating → available → attaching → in_use → detaching → available
 */
@Service
public class VolumeService {
   ...
   * Create a new volume.
   *
   * <p>Uses the capability-based storage scheduler to select the best storage for the volume. Falls
   * back to legacy provider storage mappings if the scheduler is not available. The volume is
   * initially set to "creating" status and should be transitioned to "available" by the provider
   * once ready.
   *
  ...
  public VolumeEntity createVolume(VolumeEntity volume) {
    log.info(
        "Creating volume: name={}, storageClass={}, size={}GB",
        volume.getName(),
        volume.getStorageClass(),
        volume.getSizeBytes() / (1024L * 1024L * 1024L));

    // Try capability-based scheduler first
    if (storageSchedulerService != null) {
      try {
        StorageSchedulerService.SchedulingResult result =
            storageSchedulerService.scheduleStorage(
                volume.getStorageClass(), volume.getProviderId(), volume.getSizeBytes());
        ...
        if (result.isSuccess()) {
          ...
          log.info(
              "Scheduler selected storage: name={}, type={}, score={}",
  ...
  public void updateVolumeStatus(UUID volumeId, String status) {
    VolumeEntity volume =
        volumeRepository
            .findByIdAndDeletedAtIsNull(volumeId)
   ...
   *
   * <p>Marks the volume as deleted by setting deletedAt timestamp. The volume must not be attached
   * to any resource.
   *
   ...
   *
   * <p>Expands the volume while preserving existing data. The new size must be larger than the
   * current size. The volume is set to "resizing" status and should be transitioned back to its
   * previous status by the provider.
   *
   * @param volumeId volume identifier
   ...
   *
   * <p>Creates an attachment record and transitions the volume to "in_use" status. A volume can
   * only be attached to one resource at a time.
   *
   * @param volumeId volume identifier
   ...
   *
   * <p>Marks the attachment as detached and transitions the volume back to "available" status. The
   * attachment record is preserved for audit purposes.
   *
   * @param volumeId volume identifier
   * @param resourceId resource identifier
