...
public class PluginService {
  ...
  public PluginEntity registerPlugin(Map<String, Object> manifestMap) {
    ...
    if (pluginRepository.existsByNameAndVersion(name, version)) {
      throw new PluginDuplicateException(
          "Plugin " + name + ":" + version + " is already registered");
    }
  ...
  public void deletePlugin(UUID pluginId) {
    ...
    List<ResourceTypeDefinitionEntity> rtds =
        resourceTypeDefinitionRepository.findByPluginId(pluginId);
    boolean hasActiveResources =
        rtds.stream().anyMatch(rtd -> rtd.getStatus() == ResourceTypeStatus.ACTIVE);
  ...
  private void persistCapabilities(PluginEntity plugin, Map<String, Object> spec) {
    ...
    for (Object capObj : caps) {
      ...
      com.yorel.muxon.api.enums.PluginCapabilityType capType =
          switch (type) {
            ...
            case "resource-provider" ->
                com.yorel.muxon.api.enums.PluginCapabilityType.RESOURCE_PROVIDER;
