...
import com.yorel.muxon.api.enums.RoleName;
...
import com.yorel.muxon.db.repository.SystemInitRepository;
...
/**
 * Service for managing system initialization status and bootstrap-related operations. Provides
 * methods to check bootstrap status and retrieve pre-configured bootstrap data. Uses caching
 * strategy for one-time operations to improve performance.
 */
@Slf4j
@Service
...
public class SystemInitService {
  ...
  private static final UUID SYSTEM_ADMIN_BINDING_SCOPE_ID =
      UUID.fromString("215012d9-8b1e-5dc5-b54f-89022875fe1e");

  /**
   * Get the current bootstrap status from system_init table. This is a one-time cached operation.
   *
   * @return BootstrapStatusDto containing only the system status
   */
  public BootstrapStatusDto getBootstrapStatus() {
    return systemInitRepository
        .getBootstrapStatus()
  ...
  /**
   * Check if IDP has been configured (system IDP with is_system=true). This is a one-time cached
   * operation.
   *
  ...
  public boolean checkIdpStatus() {
    return systemInitRepository
        ...
        .map(
            status ->
                status.equals(BootstrapStatus.BOOTSTRAPPED) || status.equals(BootstrapStatus.READY))
        .orElse(false);
  }

  /**
   * Check if system admin users exist (users with system:admin role). This is a one-time cached
   * operation.
   *
    ...
    return systemInitRepository
        ...
        .flatMap(
            status -> {
              ...
              List<RoleBindingEntity> adminRoleBindings =
                  roleBindingRepository.findByRoleName(RoleName.SYSTEM_ADMIN.getValue()).stream()
                      .filter(
                          rb ->
                              rb.getSubjectType() == RoleBindingSubjectType.USER
  ...
  /**
   * Check if tenant has been configured. This is a one-time cached operation.
   *
  ...
  /**
   * Get system IDP settings for the protected /api/system/init/idpsettings endpoint. Returns the
   * system IDP if it exists (is_system=true).
   *
  ...
  /**
   * Get system admin users for the protected /api/system/init/system-admin endpoint. Returns all
  ...
  /**
   * Get tenant for the protected /api/system/init/tenant endpoint. Returns the tenant if it exists.
   *
  ...
  /**
   * Add a new system admin user with system:admin role. This can be called when bootstrap has been
   * performed and additional admins need to be added.
   *
   * <p>Note: This implementation assumes users are managed by an external IDP. The user must
   * already exist in the external IDP before calling this method. The password field in the request
   * is ignored as authentication is handled by the IDP.
   *
    ...
    Optional<RoleBindingEntity> existingBinding =
        roleBindingRepository
            .findBySubjectTypeAndSubjectId(
                RoleBindingSubjectType.USER, savedUser.getId().toString())
            .stream()
            .filter(
                rb ->
                    RoleName.SYSTEM_ADMIN.getValue().equals(rb.getRoleName())
                        && rb.getScopeType() == RoleScopeType.SYSTEM)
            .findFirst();
  ...
  /**
   * Mark bootstrap as completed (READY). This is called after all required setup steps are
   * completed.
   *
