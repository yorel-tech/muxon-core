...
import com.yorel.muxon.customization.crypto.CustomizationSecretEncryptor;
...
/**
 * Spring service that wraps {@link CustomizationSecretEncryptor} to encrypt password fields in the
 * customization JSON before persisting and decrypt them before handing off to the worker.
 *
 * <p>Password field paths operated on (relative to the root of the customization JSON):
 *
 * <ul>
 *   <li>{@code windows.adminPassword}
 *   <li>{@code windows.domainJoinPassword}
 *   <li>{@code linux.users[].password}
 * </ul>
 *
...
public class CustomizationSecretService {
  ...
  public CustomizationSecretService(
    ...
    if (base64Key == null || base64Key.isBlank()) {
      log.warn(
          "muxon.customization.encryption-key is not set — customization secrets will NOT be encrypted at rest");
      this.encryptor = null;
    } else {
  ...
  /**
   * Encrypt all secret fields in the customization JSON string. Returns the mutated JSON with
   * encrypted values. If encryption is not configured the original JSON is returned unchanged.
   */
  ...
  /**
   * Decrypt all secret fields in the customization JSON string. Returns the mutated JSON with
   * decrypted (plaintext) values — for use by the worker only.
   */
  ...
  /**
   * Return a log-safe copy of the customization JSON with all secret fields replaced by {@code
   * "[REDACTED]"}. Never throws — falls back to a static placeholder on error.
   */
