...
/**
 * Validates plugin manifests against the required schema before registration. Returns structured
 * validation errors so callers can return HTTP 400/422 with detail.
 */
@Component
public class PluginManifestValidator {

  private static final Set<String> ALLOWED_CAPABILITY_TYPES =
      Set.of("runtime", "service", "resource-provider", "ui-extension");
  ...
  private void validateSpec(Map<String, Object> spec, List<String> errors) {
        ...
        if (!ALLOWED_CAPABILITY_TYPES.contains(type)) {
          errors.add(
              "spec.capabilities["
                  ...
                  + "].type '"
                  + type
                  + "' is not a valid capability type; allowed: "
    ...
    if (hasAddress && hasImage) {
      errors.add(
          "spec cannot specify both grpc.address (externally-managed) and image (platform-managed)");
    }
  }

  @SuppressWarnings("unchecked")
  private void validateResourceProviderCapability(
      Map<String, Object> cap, int idx, List<String> errors) {
