...
/**
 * Service for managing system and tenant settings. Implements tenant override pattern where
 * tenant-specific settings override system defaults.
 */
@Service
public class SystemSettingsService {
  ...
  /**
   * Get system settings (tenant_id = SYSTEM_ID) If system settings don't exist, create with default
   * values
   */
  ...
  public SystemSettings getTenantSettings(UUID tenantId) {
    ...
    Optional<SystemSettingsEntity> tenantSettingsOpt = settingsRepository.findByTenantId(tenantId);
  ...
  /**
   * Get general settings for a tenant If tenant settings don't exist, create based on system
   * defaults
   */
  public GeneralSettings getGeneralSettings(UUID tenantId) {
    ...
    if (settingsOpt.isEmpty()) {
      ...
      throw new EntityNotFoundException(
          "System settings not found. Cannot create tenant defaults.");
    }
  ...
  public GeneralSettings updateGeneralSettings(UUID tenantId, GeneralSettings settings) {
    ...
    if (settings.getDefaultTimezone() != null) {
      entity.setDefaultTimezone(settings.getDefaultTimezone());
    }
  ...
  /**
   * Get security settings for a tenant If tenant settings don't exist, create based on system
   * defaults
   */
  public SecuritySettings getSecuritySettings(UUID tenantId) {
    ...
    if (settingsOpt.isEmpty()) {
      ...
      if (systemSettingsOpt.isPresent()) {
        ...
        newEntity.setPasswordRequireUppercase(
            systemSettingsOpt.get().getPasswordRequireUppercase());
        newEntity.setPasswordRequireLowercase(
            systemSettingsOpt.get().getPasswordRequireLowercase());
  ...
  public SecuritySettings updateSecuritySettings(UUID tenantId, SecuritySettings settings) {
    ...
    if (settings.getSessionTimeoutMinutes() != null) {
      entity.setSessionTimeoutMinutes(settings.getSessionTimeoutMinutes());
    }
    if (settings.getMaxLoginAttempts() != null) {
      entity.setMaxLoginAttempts(settings.getMaxLoginAttempts());
    }
    if (settings.getLockoutDurationMinutes() != null) {
      entity.setLockoutDurationMinutes(settings.getLockoutDurationMinutes());
    }
    if (settings.getPasswordMinLength() != null) {
      entity.setPasswordMinLength(settings.getPasswordMinLength());
    }
    if (settings.getPasswordRequireUppercase() != null) {
      entity.setPasswordRequireUppercase(settings.getPasswordRequireUppercase());
    }
    if (settings.getPasswordRequireLowercase() != null) {
      entity.setPasswordRequireLowercase(settings.getPasswordRequireLowercase());
    }
    if (settings.getPasswordRequireDigit() != null) {
      entity.setPasswordRequireDigit(settings.getPasswordRequireDigit());
    }
    if (settings.getPasswordRequireSymbol() != null) {
      entity.setPasswordRequireSymbol(settings.getPasswordRequireSymbol());
    }
    if (settings.getPasswordExpiryDays() != null) {
      entity.setPasswordExpiryDays(settings.getPasswordExpiryDays());
    }
    if (settings.getApiRateLimitPerMinute() != null) {
      entity.setApiRateLimitPerMinute(settings.getApiRateLimitPerMinute());
    }
  ...
  @Transactional
  public SchemasNotificationSettings updateNotificationSettings(
      UUID tenantId, SchemasNotificationSettings settings) {
    Optional<SystemSettingsEntity> existing = settingsRepository.findByTenantId(tenantId);
    SystemSettingsEntity entity =
        existing.orElseGet(
            () -> {
    ...
    if (settings.getSlackWebhookUrl() != null) {
      entity.setSlackWebhookUrl(settings.getSlackWebhookUrl().toString());
    }
    if (settings.getWebhookEnabled() != null) {
      entity.setWebhookEnabled(settings.getWebhookEnabled());
    }
  ...
  /**
   * Get IdP settings for a tenant. For system tenant (SYSTEM_ID), returns the system IdP with full
   * details. For other tenants, returns the system IdP in read-only mode with masked secrets.
   */
  ...
  /** Update IdP settings for the system. Only system admin can update IdP settings. */
  @Transactional
  public SchemasIdpSettings updateIdpSettings(UUID tenantId, SchemasIdpSettings settings) {
    ...
    if (settings.getClientSecret() != null
        && !Constants.MASKED_SECRET.equals(settings.getClientSecret())) {
  ...
  /** Disable IdP for the system. Only system admin can disable IdP. */
  @Transactional
  ...
  public AppearanceSettings updateAppearanceSettings(UUID tenantId, AppearanceSettings settings) {
    ...
    if (settings.getSecondaryColor() != null) {
      entity.setSecondaryColor(settings.getSecondaryColor());
    }
    if (settings.getCustomCssUrl() != null) {
      entity.setCustomCssUrl(settings.getCustomCssUrl().toString());
    }
  ...
  /**
   * Merge tenant settings with system defaults Tenant values take precedence over system defaults
   */
  private SystemSettings mergeSettings(
      SystemSettingsEntity systemDefaults, SystemSettingsEntity tenantSettings) {
    ...
    merged.setDescription(
        coalesce(tenantSettings.getDescription(), systemDefaults.getDescription()));
    merged.setContactEmail(
        coalesce(tenantSettings.getContactEmail(), systemDefaults.getContactEmail()));
    merged.setContactPhone(
        coalesce(tenantSettings.getContactPhone(), systemDefaults.getContactPhone()));
    merged.setDefaultTimezone(
        coalesce(tenantSettings.getDefaultTimezone(), systemDefaults.getDefaultTimezone()));
    merged.setDefaultLocale(
        coalesce(tenantSettings.getDefaultLocale(), systemDefaults.getDefaultLocale()));

    // Security Settings - tenant override
    merged.setSessionTimeoutMinutes(
        coalesce(
            tenantSettings.getSessionTimeoutMinutes(), systemDefaults.getSessionTimeoutMinutes()));
    merged.setMaxLoginAttempts(
        coalesce(tenantSettings.getMaxLoginAttempts(), systemDefaults.getMaxLoginAttempts()));
    merged.setLockoutDurationMinutes(
        coalesce(
            tenantSettings.getLockoutDurationMinutes(),
            systemDefaults.getLockoutDurationMinutes()));
    merged.setPasswordMinLength(
        coalesce(tenantSettings.getPasswordMinLength(), systemDefaults.getPasswordMinLength()));
    merged.setPasswordRequireUppercase(
        coalesce(
            tenantSettings.getPasswordRequireUppercase(),
            systemDefaults.getPasswordRequireUppercase()));
    merged.setPasswordRequireLowercase(
        coalesce(
            tenantSettings.getPasswordRequireLowercase(),
            systemDefaults.getPasswordRequireLowercase()));
    merged.setPasswordRequireDigit(
        coalesce(
            tenantSettings.getPasswordRequireDigit(), systemDefaults.getPasswordRequireDigit()));
    merged.setPasswordRequireSymbol(
        coalesce(
            tenantSettings.getPasswordRequireSymbol(), systemDefaults.getPasswordRequireSymbol()));
    merged.setPasswordExpiryDays(
        coalesce(tenantSettings.getPasswordExpiryDays(), systemDefaults.getPasswordExpiryDays()));
    merged.setApiRateLimitPerMinute(
        coalesce(
            tenantSettings.getApiRateLimitPerMinute(), systemDefaults.getApiRateLimitPerMinute()));

    // Notification Settings - tenant override
    merged.setSmtpEnabled(
        coalesce(tenantSettings.getSmtpEnabled(), systemDefaults.getSmtpEnabled()));
    ...
    merged.setSmtpUsername(
        coalesce(tenantSettings.getSmtpUsername(), systemDefaults.getSmtpUsername()));
    merged.setSmtpPassword(
        coalesce(tenantSettings.getSmtpPassword(), systemDefaults.getSmtpPassword()));
    merged.setSmtpFromEmail(
        coalesce(tenantSettings.getSmtpFromEmail(), systemDefaults.getSmtpFromEmail()));
    merged.setSmtpUseTls(coalesce(tenantSettings.getSmtpUseTls(), systemDefaults.getSmtpUseTls()));
    merged.setSlackWebhookUrl(
        coalesce(tenantSettings.getSlackWebhookUrl(), systemDefaults.getSlackWebhookUrl()));
    merged.setWebhookEnabled(
        coalesce(tenantSettings.getWebhookEnabled(), systemDefaults.getWebhookEnabled()));
    ...
    merged.setPrimaryColor(
        coalesce(tenantSettings.getPrimaryColor(), systemDefaults.getPrimaryColor()));
    merged.setSecondaryColor(
        coalesce(tenantSettings.getSecondaryColor(), systemDefaults.getSecondaryColor()));
    merged.setCustomCssUrl(
        coalesce(tenantSettings.getCustomCssUrl(), systemDefaults.getCustomCssUrl()));
  ...
  /** Update entity from DTO Encrypt sensitive fields before saving */
  private void updateEntityFromDto(SystemSettingsEntity entity, SystemSettings dto) {
    ...
    if (dto.getGeneral() != null) {
      ...
      if (dto.getGeneral().getDescription() != null) {
        entity.setDescription(dto.getGeneral().getDescription());
      }
      if (dto.getGeneral().getContactEmail() != null) {
        entity.setContactEmail(dto.getGeneral().getContactEmail());
      }
      if (dto.getGeneral().getContactPhone() != null) {
        entity.setContactPhone(dto.getGeneral().getContactPhone());
      }
      if (dto.getGeneral().getDefaultTimezone() != null) {
        entity.setDefaultTimezone(dto.getGeneral().getDefaultTimezone());
      }
      if (dto.getGeneral().getDefaultLocale() != null) {
        entity.setDefaultLocale(dto.getGeneral().getDefaultLocale());
      }
    }

    // Security Settings
    if (dto.getSecurity() != null) {
      if (dto.getSecurity().getSessionTimeoutMinutes() != null) {
        entity.setSessionTimeoutMinutes(dto.getSecurity().getSessionTimeoutMinutes());
      }
      if (dto.getSecurity().getMaxLoginAttempts() != null) {
        entity.setMaxLoginAttempts(dto.getSecurity().getMaxLoginAttempts());
      }
      if (dto.getSecurity().getLockoutDurationMinutes() != null) {
        entity.setLockoutDurationMinutes(dto.getSecurity().getLockoutDurationMinutes());
      }
      if (dto.getSecurity().getPasswordMinLength() != null) {
        entity.setPasswordMinLength(dto.getSecurity().getPasswordMinLength());
      }
      if (dto.getSecurity().getPasswordRequireUppercase() != null) {
        entity.setPasswordRequireUppercase(dto.getSecurity().getPasswordRequireUppercase());
      }
      if (dto.getSecurity().getPasswordRequireLowercase() != null) {
        entity.setPasswordRequireLowercase(dto.getSecurity().getPasswordRequireLowercase());
      }
      if (dto.getSecurity().getPasswordRequireDigit() != null) {
        entity.setPasswordRequireDigit(dto.getSecurity().getPasswordRequireDigit());
      }
      if (dto.getSecurity().getPasswordRequireSymbol() != null) {
        entity.setPasswordRequireSymbol(dto.getSecurity().getPasswordRequireSymbol());
      }
      if (dto.getSecurity().getPasswordExpiryDays() != null) {
        entity.setPasswordExpiryDays(dto.getSecurity().getPasswordExpiryDays());
      }
      if (dto.getSecurity().getApiRateLimitPerMinute() != null) {
        entity.setApiRateLimitPerMinute(dto.getSecurity().getApiRateLimitPerMinute());
      }
    }

    // Notification Settings - encrypt password if provided and not masked
    if (dto.getNotifications() != null) {
      if (dto.getNotifications().getSmtpPassword() != null
          && !"*****".equals(dto.getNotifications().getSmtpPassword())) {
        String encrypted = EncryptionUtil.encrypt(dto.getNotifications().getSmtpPassword());
        entity.setSmtpPassword(encrypted);
      }
      if (dto.getNotifications().getSmtpEnabled() != null) {
        entity.setSmtpEnabled(dto.getNotifications().getSmtpEnabled());
      }
      if (dto.getNotifications().getSmtpHost() != null) {
        entity.setSmtpHost(dto.getNotifications().getSmtpHost());
      }
      if (dto.getNotifications().getSmtpPort() != null) {
        entity.setSmtpPort(dto.getNotifications().getSmtpPort());
      }
      if (dto.getNotifications().getSmtpUsername() != null) {
        entity.setSmtpUsername(dto.getNotifications().getSmtpUsername());
      }
      if (dto.getNotifications().getSmtpFromEmail() != null) {
        entity.setSmtpFromEmail(dto.getNotifications().getSmtpFromEmail());
      }
      if (dto.getNotifications().getSmtpUseTls() != null) {
        entity.setSmtpUseTls(dto.getNotifications().getSmtpUseTls());
      }
      if (dto.getNotifications().getSlackWebhookUrl() != null) {
        entity.setSlackWebhookUrl(dto.getNotifications().getSlackWebhookUrl().toString());
      }
      if (dto.getNotifications().getWebhookEnabled() != null) {
        entity.setWebhookEnabled(dto.getNotifications().getWebhookEnabled());
      }
    }

    // Appearance Settings
    if (dto.getAppearance() != null) {
      if (dto.getAppearance().getTheme() != null) {
        entity.setTheme(dto.getAppearance().getTheme().name());
      }
      if (dto.getAppearance().getLogoUrl() != null) {
        entity.setLogoUrl(dto.getAppearance().getLogoUrl().toString());
      }
      if (dto.getAppearance().getFaviconUrl() != null) {
        entity.setFaviconUrl(dto.getAppearance().getFaviconUrl().toString());
      }
      if (dto.getAppearance().getPrimaryColor() != null) {
        entity.setPrimaryColor(dto.getAppearance().getPrimaryColor());
      }
      if (dto.getAppearance().getSecondaryColor() != null) {
        entity.setSecondaryColor(dto.getAppearance().getSecondaryColor());
      }
      if (dto.getAppearance().getCustomCssUrl() != null) {
        entity.setCustomCssUrl(dto.getAppearance().getCustomCssUrl().toString());
      }
    }
  }
  ...
  private SystemSettingsEntity createDefaultSystemSettings() {
    ...
    entity.setName("Muxon Cloud Platform");
    entity.setDescription("Multi-tenant cloud management platform");
    entity.setContactEmail("admin@muxon.example");
  ...
  public void sendTestEmail(UUID tenantId, TestEmailRequest request) {
    ...
    System.out.println("Sending test email to: " + request.getTo() + "from tenant: " + tenantId);
  }
  ...
  public IdpValidationResponse validateIdpConfiguration(IdpValidationRequest request) {
    ...
    if (request.getIssuerUrl() == null
        || !request.getIssuerUrl().toString().matches("^https?://.*")) {
