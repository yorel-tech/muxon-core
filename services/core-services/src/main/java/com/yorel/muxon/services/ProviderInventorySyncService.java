...
/**
 * Enqueues full provider inventory sync (capabilities where applicable, node cluster + nodes,
 * storage).
 */
@Service
  ...
  public UUID enqueueInventorySync(UUID providerId) {
    ...
    if (!SUPPORTED_TYPES.contains(providerType)) {
      throw new IllegalArgumentException(
          "Provider inventory sync is not supported for type: " + providerType);
    }
    ...
    taskRequest.setMetadata(
        Map.of("kind", "PROVIDER_INVENTORY_SYNC", "providerId", providerId.toString()));
    ...
    if (superseded > 0) {
      log.info(
          "Superseded {} stale PENDING inventory-sync queue row(s) for provider {}",
          superseded,
          providerId);
    }
    ...
    log.info(
        "Provider inventory sync enqueued: providerId={}, jobId={}, commandId={}, executionTimeoutSeconds={}",
        providerId,
        jobId,
        commandId,
