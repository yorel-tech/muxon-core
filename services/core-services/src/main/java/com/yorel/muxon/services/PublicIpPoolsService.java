...
import com.yorel.muxon.db.repository.PublicIpPoolRepository;
...
public class PublicIpPoolsService {
  ...
  @Autowired private PublicIpPoolRepository poolRepository;
  ...
  @Transactional
  public PublicIpPoolEntity create(
      UUID datacenterId, String cidr, String name, String description) {
    DatacenterEntity dc =
    ...
    if (poolRepository.existsByDatacenterIdAndCidr(datacenterId, cidr)) {
      throw new IllegalStateException(
          "CONFLICT: Public IP pool with CIDR '" + cidr + "' already exists in this datacenter");
    }
