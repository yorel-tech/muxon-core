...
public class ContentItemApiConverter {
  ...
  public ContentItem toApi(ContentItemEntity entity) {
    String type =
        entity.getContentType() == null ? "" : entity.getContentType().toLowerCase(Locale.ROOT);
    Object api;
    ...
    switch (type) {
      case "vm_template" -> {
        ...
        if (entity.getTemplateSpec() != null) {
          vm.setTemplateSpec(
              objectMapper.convertValue(entity.getTemplateSpec(), VmTemplateSpec.class));
        }
        api = vm;
      }
  ...
  private static void fillBase(Object api, ContentItemEntity entity) {
    if (api instanceof VmTemplateContentItem x) {
      ...
      throw new IllegalStateException(
          "Unsupported content item api type: " + api.getClass().getName());
    }
  }
