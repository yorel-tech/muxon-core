...
import com.yorel.muxon.api.VmManagementApi;
...
import com.yorel.muxon.services.VmsService;
...
public class VmsController implements VmManagementApi {
  ...
  @Autowired private VmsService vmsService;
  ...
  public ResponseEntity<VmListResponse> listVms(
      UUID tenantId,
      Integer page,
      Integer perPage,
      VmStatus status,
      ...
      String sort,
      String order) {
    VmListResponse result =
        vmsService.listVms(
            tenantId, page, perPage, status, tenantDatacenterGrantId, tags, sort, order);
  ...
  @Override
  @RequiresPermission(Permission.VM_MANAGE)
  public ResponseEntity<VmOperationResponse> attachVmIso(
      UUID tenantId, UUID vmId, VmIsoAttachRequest vmIsoAttachRequest) {
    return ResponseEntity.accepted()
        .body(vmsService.attachVmIso(tenantId, vmId, vmIsoAttachRequest));
  }

  @Override
  @RequiresPermission(Permission.VM_MANAGE)
  public ResponseEntity<VmOperationResponse> detachVmIso(
      UUID tenantId, UUID vmId, VmIsoDetachRequest vmIsoDetachRequest) {
    return ResponseEntity.accepted()
        .body(vmsService.detachVmIso(tenantId, vmId, vmIsoDetachRequest));
  }
  ...
  public ResponseEntity<VmPublishTemplateResponse> publishVmAsTemplate(
    ...
    return ResponseEntity.accepted()
        .body(vmsService.publishVmAsTemplate(tenantId, vmId, vmPublishTemplateRequest));
  }
}
