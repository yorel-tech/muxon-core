...
/**
 * Applies {@code content_library_distribution} / {@code content_item_distribution} updates from
 * worker-published {@link com.yorel.muxon.spi.queue.EntityEventQueue} events.
 */
@Component
public class ContentLibraryEntityEventHandler {
  ...
  @Autowired private ContentLibraryDistributionRepository distributionRepository;

  @Autowired private ContentItemDistributionRepository itemDistributionRepository;
  ...
  public void onItemUpdated(EntityEventMessage event) {
    ...
    if (contentItemId == null) {
      log.warn(
          "content_library_distribution.item_updated missing contentItemId for distribution {}",
          distributionId);
      return;
    }
    ...
    if (status == null) {
      log.warn(
          "content_library_distribution.item_updated missing status for item {}", contentItemId);
      return;
    }
  ...
  private static void applyItemRow(
      ContentItemDistributionEntity row, Map<String, Object> p, String status) {
  ...
  private void recomputeDistributionProgress(UUID distributionId) {
    ...
    long terminal =
        itemDistributionRepository.countByDistributionIdAndStatus(distributionId, "READY")
    ...
    distributionRepository
        ...
        .ifPresentOrElse(
            d -> {
              ...
              distributionRepository.save(d);
              log.warn(
                  "Content library distribution {} marked failed: {}",
                  distributionId,
                  d.getErrorMessage());
            },
