...
import com.yorel.muxon.db.model.BucketEntity;
import com.yorel.muxon.db.repository.BucketRepository;
 ...
 *
 * <p>Handles bucket creation, deletion, and queries. Buckets provide object storage for
 * unstructured data such as backups, media files, logs, and archives. Each bucket is S3-compatible
 * and can be accessed using standard S3 APIs and tools.
 *
 * <p>Bucket names must be unique within a workspace.
 */
@Service
public class BucketService {
   ...
   *
   * <p>Validates that the bucket name is unique within the workspace. Bucket names must follow S3
   * naming conventions.
   *
  ...
  public BucketEntity createBucket(BucketEntity bucket) {
    log.info(
        "Creating bucket: name={}, storageClass={}", bucket.getName(), bucket.getStorageClass());

    Optional<BucketEntity> existing =
        bucketRepository.findByNameAndWorkspaceIdAndDeletedAtIsNull(
            bucket.getName(), bucket.getWorkspaceId());
   ...
   *
   * <p>Marks the bucket as deleted. Note: The provider should handle deletion of bucket contents
  ...
  public void deleteBucket(UUID bucketId) {
    BucketEntity bucket =
        bucketRepository
            .findByIdAndDeletedAtIsNull(bucketId)
