...
public class ActionLinkService {
  ...
  /**
   * Get the current authenticated user from SecurityContext. Public so SpEL in @Cacheable key
   * expressions can call it on the proxy.
   *
  ...
  @Cacheable(
      value = "actionLinks",
      key =
          "'provider-links-' + #provider.getId() + '-' + @authorizationService.getCachedPermissions(@actionLinkService.getCurrentUser())")
  ...
  @Cacheable(
      value = "actionLinks",
      key =
          "'cluster-links-' + #cluster.getId() + '-' + @authorizationService.getCachedPermissions(@actionLinkService.getCurrentUser())")
  ...
  @Cacheable(
      value = "actionLinks",
      key =
          "'node-links-' + #node.getId() + '-' + @authorizationService.getCachedPermissions(@actionLinkService.getCurrentUser())")
  ...
  @Cacheable(
      value = "actionLinks",
      key =
          "'datacenter-links-' + #datacenterId + '-' + @authorizationService.getCachedPermissions(@actionLinkService.getCurrentUser())")
  ...
  @Cacheable(
      value = "actionLinks",
      key =
          "'tenant-links-' + #tenantId + '-' + @authorizationService.getCachedPermissions(@actionLinkService.getCurrentUser())")
  ...
  private List<Link> buildLinksForResource(
      Class<?> resourceType, String resourceId, Object resource, UserPrincipal user) {
    ...
    for (ResourceActionDescriptor descriptor : resourceActionRegistry.getActionsFor(resourceType)) {
      ...
      String href = descriptor.pathTemplate().replace("{" + descriptor.idParam() + "}", resourceId);
      ...
      if (resourceType.equals(Provider.class)
          && resource instanceof ProviderEntity providerEntity) {
        ...
        if ("sync".equals(descriptor.rel())
            && enabled
            && "OFFLINE".equals(providerEntity.getStatus())) {
          enabled = false;
          reason = "Provider is offline or missing sync permission";
        }
      } else if (resourceType.equals(NodeCluster.class)
          && resource instanceof NodeClusterEntity clusterEntity) {
      ...
      switch (descriptor.rel()) {
        ...
        case "delete":
          ...
          break;
        default:
          link = LinkUtil.custom(descriptor.rel(), href, httpMethod.name(), title, enabled, reason);
          break;
      }
