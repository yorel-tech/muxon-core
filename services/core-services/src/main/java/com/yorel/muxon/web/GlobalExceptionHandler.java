...
import com.yorel.muxon.services.AuditService;
import com.yorel.muxon.web.dto.ErrorResponse;
import com.yorel.muxon.web.exceptions.ActionNotAllowedException;
import com.yorel.muxon.web.exceptions.PermissionDeniedException;
import com.yorel.muxon.web.exceptions.ResourceNotFoundException;
...
/**
 * Global exception handler for REST controllers. Provides consistent error responses and audit
...
public class GlobalExceptionHandler {
  ...
  /** Handle permission denied exceptions. Returns HTTP 403 Forbidden. */
  ...
  public ResponseEntity<ErrorResponse> handlePermissionDenied(
    ...
    ErrorResponse error =
        new ErrorResponse("PERMISSION_DENIED", ex.getMessage(), System.currentTimeMillis());

    return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
  }

  /** Handle resource not found exceptions. Returns HTTP 404 Not Found. */
  ...
  public ResponseEntity<ErrorResponse> handleResourceNotFound(
    ...
    ErrorResponse error =
        new ErrorResponse("RESOURCE_NOT_FOUND", ex.getMessage(), System.currentTimeMillis());

    return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
  }

  /** Handle entity not found exceptions from services. Returns HTTP 404 Not Found. */
  ...
  /** Handle action not allowed exceptions. Returns HTTP 409 Conflict. */
  ...
  public ResponseEntity<ErrorResponse> handleActionNotAllowed(
    ...
    ErrorResponse error =
        new ErrorResponse("ACTION_NOT_ALLOWED", ex.getMessage(), System.currentTimeMillis());

    return ResponseEntity.status(HttpStatus.CONFLICT).body(error);
  }

  /**
   * Handle validation errors from @Valid annotations. Returns HTTP 400 Bad Request with field-level
   * error details.
   */
  ...
  /**
   * JSON body could not be deserialized (schema mismatch, unknown fields if enforced, malformed
   * JSON).
   */
  ...
  public ResponseEntity<ErrorResponse> handleHttpMessageNotReadable(
    ...
    logger.warn(
        "Unreadable request body: {} - Request: {}", rootMessage, request.getDescription(false));
  ...
  /** Handle illegal argument / business validation exceptions. Returns HTTP 400 Bad Request. */
  ...
  public ResponseEntity<ErrorResponse> handleIllegalArgument(
    ...
    ErrorResponse error =
        new ErrorResponse("BAD_REQUEST", ex.getMessage(), System.currentTimeMillis());

    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
  }

  /**
   * Business / lifecycle violations (e.g. VM not running, console resolve incomplete). Exposed to
   * clients so UIs are not stuck with a generic 500 body.
   */
  ...
  public ResponseEntity<ErrorResponse> handleIllegalState(
    ...
    ErrorResponse error = new ErrorResponse("ILLEGAL_STATE", msg, System.currentTimeMillis());

    return ResponseEntity.status(HttpStatus.CONFLICT).body(error);
  }

  /** Handle all other uncaught exceptions. Returns HTTP 500 Internal Server Error. */
