...
public class ContentItemUploadCompletionHandler {
   ...
   *
   * <p>The upload API is item-scoped; each completed upload session is treated as one disk file,
   * assigned in the order uploads are completed: disk-0, disk-1, ...
   */
  public void finalizeVmTemplateDiskUpload(
    ...
    if (baseRelative == null || baseRelative.isBlank()) {
      throw new IllegalStateException(
          "providerRelativePath is required to place uploaded artifacts");
    }
    ...
    if (contentStorageS3Uploader.isS3(storage)) {
      ...
      Path root =
          contentStoragePathResolver
              .artifactRootForLibrary(library.getId())
              .toAbsolutePath()
              .normalize();
    ...
    Map<String, Object> updated =
        objectMapper.convertValue(spec, new TypeReference<Map<String, Object>>() {});
    item.setTemplateSpec(updated);

    // Update aggregate size/checksum fields (best-effort; checksum refers to last uploaded file for
    // now)
    ...
    if (isSpecComplete(spec)
        && allDiskFilesPresent(storage, library.getId(), baseDirRelative, disks)) {
  ...
  private int nextDiskIndex(
      ContentStorageEntity storage,
      java.util.UUID libraryId,
      String baseDirRelative,
      java.util.UUID itemId) {
    if (contentStorageS3Uploader.isS3(storage)) {
      // For S3 we don't have a cheap list in current uploader; rely on count stored in DB via
      // templateSpec.path.
      ContentItemEntity refreshed =
          contentItemRepository
              .findById(itemId)
              .orElseThrow(() -> new EntityNotFoundException("Content item not found: " + itemId));
      VmTemplateSpec spec =
          objectMapper.convertValue(refreshed.getTemplateSpec(), VmTemplateSpec.class);
      long count =
          Optional.ofNullable(spec.getSpec()).map(s -> s.getDisks()).stream()
              .flatMap(List::stream)
    ...
    try (Stream<Path> s = Files.list(disksDir)) {
      return (int)
          s.filter(
                  p ->
                      p.getFileName().toString().startsWith("disk-")
                          && p.getFileName().toString().endsWith(".qcow2"))
              .count();
    } catch (Exception e) {
  ...
  private void writeTemplateJsonAndChecksums(
      ContentStorageEntity storage,
      java.util.UUID libraryId,
      String baseDirRelative,
      VmTemplateSpec spec) {
    ...
    try {
      ...
      Files.writeString(
          templateJson, objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(spec));
