...
import com.yorel.muxon.db.repository.SnapshotRepository;
 ...
 *
 * <p>Handles snapshot creation, deletion, restore, and clone operations. Supports immutable
 * snapshots with retention periods for compliance and WORM (Write Once Read Many) requirements.
 *
 * <p>Snapshot lifecycle: creating → available → deleting → deleted
 */
@Service
public class SnapshotService {
  ...
  public SnapshotService(SnapshotRepository snapshotRepository, VolumeRepository volumeRepository) {
   ...
   *
   * <p>Captures the current state of the volume. The snapshot size is set to match the volume size.
   * The snapshot is initially in "creating" status and should be transitioned to "available" by the
   * provider once ready.
   *
  ...
  public SnapshotEntity createSnapshot(SnapshotEntity snapshot) {
    VolumeEntity volume =
        volumeRepository
            .findByIdAndDeletedAtIsNull(snapshot.getVolumeId())
            .orElseThrow(
                () -> new IllegalArgumentException("Volume not found: " + snapshot.getVolumeId()));
    ...
    log.info("Creating snapshot: name={}, volumeId={}", snapshot.getName(), snapshot.getVolumeId());
  ...
  public void updateSnapshotStatus(UUID snapshotId, String status) {
    SnapshotEntity snapshot =
        snapshotRepository
            .findByIdAndDeletedAtIsNull(snapshotId)
   ...
   *
   * <p>Marks the snapshot as deleted. Immutable snapshots cannot be deleted before their retention
   * period expires (compliance/WORM enforcement).
   *
  ...
  public void deleteSnapshot(UUID snapshotId) {
    ...
    if (snapshot.getImmutable()
        && snapshot.getRetentionUntil() != null
   ...
   *
   * <p>Creates a new volume with the contents of the snapshot. The new volume inherits properties
   * from the source volume (workspace, encryption, etc.) but can use a different storage class if
   * specified.
   *
   * @param snapshotId snapshot identifier
   ...
   * ZFS).
   *
   * <p>Note: Currently implemented as an alias to restoreFromSnapshot. Provider-specific
   * implementations should optimize this for CoW.
   *
   * @param snapshotId snapshot identifier
   * @param volumeName name for the new volume
   * @param storageClass storage class for new volume (null to use source volume's class)
