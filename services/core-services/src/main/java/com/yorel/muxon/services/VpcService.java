...
import com.yorel.muxon.db.model.VpcEntity.VpcStatus;
...
public class VpcService {
  ...
  private static final String[] PRIVATE_CIDRS = {
    "10.",
    "172.16.",
    "172.17.",
    "172.18.",
  ...
  @Transactional
  public VpcEntity create(
      UUID tenantId, String name, String cidr, String description, Map<String, String> metadata) {
    if (cidr != null && !isPrivateCidr(cidr)) {
      throw new IllegalArgumentException(
          "INVALID_CIDR: VPC CIDR must be a private (RFC 1918) range");
    }
            ...
            policy -> {
              ...
              if (count >= policy.getMaxVpcs()) {
                throw new IllegalStateException(
                    "QUOTA_EXCEEDED: Tenant has reached max_vpcs limit of " + policy.getMaxVpcs());
              }
            });

    if (vpcRepository.existsByTenantIdAndName(tenantId, name)) {
      throw new IllegalStateException(
          "CONFLICT: VPC with name '" + name + "' already exists for this tenant");
    }
  ...
  public void delete(UUID vpcId) {
    ...
    if (activeSubnets > 0) {
      throw new IllegalStateException(
          "CONFLICT: VPC has " + activeSubnets + " active subnet(s); delete them first");
    }
