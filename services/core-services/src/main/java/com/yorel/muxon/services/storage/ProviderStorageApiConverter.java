...
public class ProviderStorageApiConverter {
  ...
  public ProviderStorage toApi(
      ProviderStorageEntity entity, ProviderStorageMappingService.MappingBatchContext ctx) {
    ProviderType providerType =
        ProviderType.valueOf(entity.getProviderType().toUpperCase(Locale.ROOT));
    ProviderStorage api =
        new ProviderStorage(
            entity.getId(),
  ...
  private static ProviderStorageMetrics mapStorageMetrics(Map<String, Object> raw) {
    ...
    for (Map.Entry<String, Object> e : m.entrySet()) {
      String k = e.getKey();
      if (k.equals("total_gb")
          || k.equals("free_gb")
          || k.equals("used_gb")
          || k.equals("estimated_iops")
          || k.equals("latency_ms")) {
        continue;
      }
