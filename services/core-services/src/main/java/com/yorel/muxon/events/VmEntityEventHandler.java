...
/**
 * Handles entity events for VM entities. Updates the {@code vm} table based on events published by
 * the worker via {@code EntityEventQueue}.
 *
  ...
  public void onMigrated(EntityEventMessage event) {
    updateVm(
        ...
        vm -> {
          ...
          if (nodeId != null) {
            try {
              vm.setNodeId(UUID.fromString(nodeId.toString()));
  ...
  public void onCustomizationUpdate(EntityEventMessage event) {
    updateVm(
        ...
        vm -> {
          ...
          String phase =
              event.payload().containsKey("customizationPhase")
                  ? event.payload().get("customizationPhase").toString()
                  : null;
