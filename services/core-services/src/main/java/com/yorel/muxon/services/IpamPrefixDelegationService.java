...
public class IpamPrefixDelegationService {
  ...
  @Autowired private IpamPrefixDelegationRepository delegationRepository;
  ...
  @Transactional
  public IpamPrefixDelegationEntity create(
      UUID subnetId, UUID stackId, UUID nodeVmId, String delegatedCidr) {
    SubnetEntity subnet =
        subnetRepository
            .findById(subnetId)
    ...
    if (delegationRepository.existsOverlappingDelegation(subnetId, delegatedCidr, nullUUID)) {
      throw new IllegalStateException(
          "CONFLICT: Delegated CIDR " + delegatedCidr + " overlaps with existing delegation");
    }
  ...
  public void release(UUID delegationId) {
    IpamPrefixDelegationEntity entity =
        delegationRepository
            .findById(delegationId)
            .orElseThrow(
                () -> new EntityNotFoundException("Delegation not found: " + delegationId));

    // Remove route table entry
    if (entity.getSubnet().getRouteTable() != null) {
      try {
        routeTablesService.listEntries(entity.getSubnet().getRouteTable().getId()).stream()
            .filter(
                e ->
                    entity.getNodeVmId().equals(e.getTargetId())
  ...
  public void reconcileRouteErrors() {
    findRouteErrorDelegations()
        .forEach(
            delegation -> {
              tryCreateRouteEntry(
                  delegation,
                  delegation.getSubnet(),
                  delegation.getNodeVmId(),
                  delegation.getDelegatedCidr());
              delegationRepository.save(delegation);
            });
  }

  private void tryCreateRouteEntry(
      IpamPrefixDelegationEntity delegation,
      SubnetEntity subnet,
      UUID nodeVmId,
      String delegatedCidr) {
    try {
      if (subnet.getRouteTable() != null) {
        routeTablesService.addEntry(
            subnet.getRouteTable().getId(), delegatedCidr, RouteTargetType.INSTANCE, nodeVmId);
