 ...
 *
 * <p>This is the <strong>only</strong> component in {@code core-services} that updates entity
 * tables in response to worker-reported state changes. Previously this was done by the
 * orchestrator's legacy VM polling path via direct {@code vmRepository.save()} calls — that
 * violation is now removed.
 *
...
public class EntityEventProcessor {
  ...
  @Autowired private VmEntityEventHandler vmEventHandler;

  @Autowired private ContentLibraryEntityEventHandler contentLibraryEntityEventHandler;
  ...
  public void processEntityEvents() {
    ...
    for (EntityEventMessage event : events) {
      try {
        ...
        log.error(
            "Failed to process entity event {} (type={}): {}",
            event.id(),
            event.eventType(),
            ex.getMessage(),
            ex);
  ...
  private void dispatchContentLibraryEvent(EntityEventMessage event) {
    switch (event.eventType()) {
      case EntityEventTypes.CL_DISTRIBUTION_ITEM_UPDATED ->
          contentLibraryEntityEventHandler.onItemUpdated(event);
      case EntityEventTypes.CL_DISTRIBUTION_COMPLETED ->
          contentLibraryEntityEventHandler.onCompleted(event);
      case EntityEventTypes.CL_DISTRIBUTION_FAILED ->
          contentLibraryEntityEventHandler.onFailed(event);
      default ->
          log.debug(
  ...
  private void dispatchVmEvent(EntityEventMessage event) {
    switch (event.eventType()) {
      ...
      case EntityEventTypes.VM_CUSTOMIZATION_STATUS_UPDATED,
          ...
          vmEventHandler.onCustomizationUpdate(event);
      default ->
          log.debug(
              "Unhandled VM event type '{}' for entity {}", event.eventType(), event.entityId());
    }
  }
}
