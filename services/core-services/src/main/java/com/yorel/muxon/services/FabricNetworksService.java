...
import com.yorel.muxon.db.model.FabricNetworkEntity.FabricNetworkRole;
import com.yorel.muxon.db.model.FabricNetworkEntity.FabricNetworkStatus;
import com.yorel.muxon.db.model.FabricNetworkEntity.FabricNetworkType;
...
public class FabricNetworksService {
  ...
  @Transactional
  public FabricNetworkEntity create(
      UUID datacenterId,
      String name,
      FabricNetworkType type,
      FabricNetworkRole role,
      String externalId,
      Integer vlanId,
      String config) {
    if (role == null) {
    ...
    if (fabricNetworkRepository.existsByNodeClusterIdAndName(cluster.getId(), name)) {
      throw new IllegalStateException(
          "CONFLICT: Fabric network with name '" + name + "' already exists in this cluster");
    }
  ...
  public FabricNetworkEntity getById(UUID fabricNetworkId) {
    return fabricNetworkRepository
        .findById(fabricNetworkId)
        .orElseThrow(
            () -> new EntityNotFoundException("Fabric network not found: " + fabricNetworkId));
  }
