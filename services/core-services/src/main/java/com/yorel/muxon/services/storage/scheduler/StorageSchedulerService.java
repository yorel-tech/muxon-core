 ...
 *
 * <p>This service implements the storage scheduling workflow:
 *
 * <ol>
 *   <li>Check for manual overrides
 *   <li>Filter candidates by capabilities and constraints
 *   <li>Score candidates using scoring algorithm
 *   <li>Select best candidate
 * </ol>
 */
@Service
public class StorageSchedulerService {
   ...
   *
   * <p>Selects the best provider storage based on storage class requirements.
   *
  ...
  public SchedulingResult scheduleStorage(
      String storageClassName, UUID providerId, long sizeBytes) {

    log.info(
        "Scheduling storage: class={}, provider={}, size={}GB",
        storageClassName,
        providerId,
    ...
    if (candidates.isEmpty()) {
      ...
      log.debug(
          "Found {} enabled storage candidates for provider {}", candidates.size(), providerId);
    } else {
    ...
    log.info(
        "Selected storage: name={}, type={}, score={}",
  ...
  /**
   * All enabled provider pools for {@code providerId} that match {@code storageClassName}
   * (capabilities, constraints, and optional storage overrides). Used when replicating content into
   * every eligible pool.
   */
  public List<ProviderStorageEntity> findMatchingStoragePoolsForProvider(
    ...
    if (candidates.isEmpty()) {
      candidates =
          new ArrayList<>(providerStorageRepository.findByProviderIdAndEnabled(providerId, true));
    }
    if (candidates.isEmpty()) {
      return List.of();
    }
  ...
  public static class SchedulingResult {
    ...
    public static SchedulingResult success(
        ProviderStorageEntity storage, Map<String, Object> metadata) {
