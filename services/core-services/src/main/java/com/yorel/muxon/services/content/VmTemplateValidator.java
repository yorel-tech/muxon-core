...
public class VmTemplateValidator {
  ...
  public void validate(VmTemplateSpec spec) {
    ...
    if (spec.getMetadata() == null
        || spec.getMetadata().getName() == null
        || spec.getMetadata().getName().isBlank()) {
    ...
    if (spec.getSpec().getCompute().getCpuCores() == null
        || spec.getSpec().getCompute().getCpuCores() < 1) {
      throw new IllegalArgumentException("templateSpec.spec.compute.cpuCores must be >= 1");
    }
    if (spec.getSpec().getCompute().getMemoryMB() == null
        || spec.getSpec().getCompute().getMemoryMB() < 512) {
    ...
    if (spec.getSpec().getNetwork() == null || spec.getSpec().getNetwork().isEmpty()) {
      throw new IllegalArgumentException(
          "templateSpec.spec.network must have at least 1 interface");
    }
  ...
  private static void validateDisks(List<VmTemplateDiskSpec> disks) {
    ...
    for (int i = 1; i <= max; i++) {
      if (!bootOrders.contains(i)) {
        throw new IllegalArgumentException(
            "bootOrder values must be sequential starting at 1 (missing " + i + ")");
      }
    }
  }
}
