 ...
 *
 * <ul>
 *   <li>Per-plugin channel lifecycle (create/reuse/shutdown)
 *   <li>Per-capability deadline interceptors
 *   <li>Circuit breaker: opens after N consecutive non-OK responses
 *   <li>{@link TenantContextInterceptor} attached to all outbound calls
 *   <li>mTLS stub (Phase 1: self-signed cert stored in plugin manifest JSONB)
 * </ul>
 */
@Component
public class PluginGrpcChannelFactory {
  ...
  private ManagedChannel buildChannel(PluginEntity plugin) {
    ...
    log.info(
        "Building gRPC channel for plugin {}:{} → {}:{}",
        plugin.getName(),
        plugin.getVersion(),
        host,
        port);
  ...
  public boolean checkHealth(PluginEntity plugin) {
    ...
    try {
      ...
      if (cb.isOpen()) {
        log.warn(
            "Circuit breaker opened for plugin {} after {} consecutive failures",
            plugin.getName(),
  ...
  public boolean reconcileResource(
    ...
    log.debug(
        "reconcileResource kind={} resourceId={} plugin={}", kind, resourceId, plugin.getName());
    return true;
  }

  // ── CatalogContribution ──────────────────────────────────────────────────

  public Map<String, Object> validateCatalogConfig(
      PluginEntity plugin, String itemName, String configJson) {
    log.debug("validateCatalogConfig plugin={} item={}", plugin.getName(), itemName);
    return Map.of("valid", true, "errors", java.util.List.of());
  }

  public boolean provisionCatalogItem(
      PluginEntity plugin, UUID serviceInstanceId, String itemName, String configJson) {
    log.debug(
        "provisionCatalogItem plugin={} item={} instanceId={}",
        plugin.getName(),
        itemName,
        serviceInstanceId);
    return true;
  ...
  public boolean deprovisionCatalogItem(
      PluginEntity plugin, UUID serviceInstanceId, String externalId) {
    log.debug(
        "deprovisionCatalogItem plugin={} instanceId={}", plugin.getName(), serviceInstanceId);
    return true;
  }
