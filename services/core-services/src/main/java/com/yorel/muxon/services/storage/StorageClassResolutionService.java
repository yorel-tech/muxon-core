 ...
 *
 * <p>This service bridges the gap between user-facing storage class names (e.g., "fast-ssd",
 * "balanced") and provider-specific storage implementations. It handles storage class validation,
 * provider mapping resolution, and storage class management.
 *
 * <p>Example: When a user requests a "fast-ssd" volume, this service determines which provider
 * backend to use (e.g., Ceph RBD pool "ssd_pool" for Libvirt, or ZFS dataset "tank/ssd" for
 * Proxmox).
 */
@Service
public class StorageClassResolutionService {
   ...
   *
   * <p>Returns the enabled mapping that connects a storage class to a specific provider's backend
   * configuration. The mapping includes backend type (e.g., "ceph-rbd", "zfs") and
   ...
   *
   * <p>Returns all enabled mappings ordered by priority (highest first). Used when multiple
   * providers can fulfill a storage class request.
   *
   * @param storageClassName storage class name
   ...
   *
   * <p>Validates that both the storage class exists and has an enabled mapping to the specified
   * provider.
   *
   * @param storageClassName storage class name
   * @param providerId provider identifier
  ...
  public ProviderStorageMappingEntity createProviderMapping(ProviderStorageMappingEntity mapping) {
