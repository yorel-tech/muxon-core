 ...
 *
 * <p>Scores storage based on:
 *
 * <ul>
 *   <li>IOPS (40%)
 *   <li>Free capacity ratio (40%)
 *   <li>Latency estimate (20% penalty)
 * </ul>
 */
@Component
public class SimpleStorageScorer {
  ...
  public List<ScoredStorage> scoreAndRank(
    ...
    log.debug(
        "Scored {} candidates, top score: {}",
        scored.size(),
        scored.isEmpty() ? 0 : scored.get(0).getScore());
  ...
  private double calculateScore(ProviderStorageEntity storage, SchedulerContext context) {
    ...
    double totalScore =
        (WEIGHT_IOPS * iopsScore)
            + (WEIGHT_CAPACITY * capacityScore)
            - (WEIGHT_LATENCY * latencyScore);

    log.debug(
        "Storage {} score: total={}, iops={}, capacity={}, latency={}",
        storage.getName(),
        totalScore,
        iopsScore,
        capacityScore,
  ...
  /** Normalize IOPS to 0-1 scale. Assumes max IOPS of 100,000. */
  ...
  /** Calculate latency penalty based on storage type. Lower values are better (less penalty). */
