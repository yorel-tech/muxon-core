...
import com.scal.muxon.db.repository.PluginUiModuleRepository;
 ...
 * <ol>
 *   <li>The owning plugin is {@code ACTIVE}.</li>
 *   <li>The plugin is published to the requesting tenant (publication scope check).</li>
 *   <li>All {@code requiredPermissions} are satisfied by the user's effective permission set.</li>
 * </ol>
 *
 * <p>Responses are cached per (tenantId + permissions fingerprint) with a short TTL
 * to avoid per-request DB queries (cache configured in application.yaml via Caffeine).
 */
@Service
public class UiExtensionService {
    ...
    public List<UiModuleEntry> listExtensionsForUser(UUID tenantId, Set<String> userPermissions) {
        ...
        List<UUID> publishedPluginIds = activePlugins.stream()
    ...
    /**
     * Publication scope check. In the base implementation all ACTIVE plugins are
     * published to all tenants (publication scope ALL by default).
     * Enterprise overrides this to consult plugin_publications.
     */
