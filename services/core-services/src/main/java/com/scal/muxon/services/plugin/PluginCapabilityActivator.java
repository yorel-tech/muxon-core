...
public class PluginCapabilityActivator {
    ...
    /**
     * Calls DescribeCapabilities on the plugin, validates the response against manifest
     * declarations, then registers resource types, RBAC permissions, UI modules, and
     * catalog contributions. Throws if any mismatch is found — the entire activation is aborted.
     */
    @Transactional
        ...
        for (PluginCapabilityEntity cap : capabilities) {
            ...
            if (!implementedServices.contains(expectedService)) {
                throw new PluginRegistrationException(
                        "Plugin " + plugin.getName() + " declared capability " + cap.getCapabilityType()
                        + " but DescribeCapabilities did not list " + expectedService);
            }
    ...
    private void activateResourceProvider(PluginEntity plugin, PluginCapabilityEntity cap) {
        ...
        for (Object kindObj : kinds) {
            ...
            Optional<ResourceTypeDefinitionEntity> existing = resourceTypeDefinitionRepository.findByKind(kind);
            if (existing.isPresent() && existing.get().getStatus() == ResourceTypeStatus.ACTIVE
                    && !existing.get().getPlugin().getId().equals(plugin.getId())) {
                throw new PluginRegistrationException(
                        "Kind '" + kind + "' is already owned by an active plugin: "
                        + existing.get().getPlugin().getName());
            }

            ResourceTypeDefinitionEntity rtd = existing
            ...
            log.info("Registered resource kind '{}' for plugin {}:{}", kind, plugin.getName(), plugin.getVersion());
        }
    }
    ...
    private void activateUiExtension(PluginEntity plugin, PluginCapabilityEntity cap) {
        ...
        for (Object modObj : modules) {
            ...
            PluginUiModuleEntity entity = uiModuleRepository.findByPluginId(plugin.getId())
                    .stream()
            ...
            entity.setRequiredPermissions(permsObj instanceof List<?> perms
                    ? perms.stream().map(Object::toString).toList() : List.of());

            Object kindsObj = mod.get("targetKinds");
            entity.setTargetKinds(kindsObj instanceof List<?> kinds
                    ? kinds.stream().map(Object::toString).toList() : List.of());

            uiModuleRepository.save(entity);
            log.info("Registered UI module '{}' for plugin {}:{}", moduleId, plugin.getName(), plugin.getVersion());
        }
    }
    ...
    private void activateCatalogContributions(PluginEntity plugin, PluginCapabilityEntity cap) {
        ...
        CatalogContributionStatus initialStatus = catalogApprovalRequired
        ...
        for (Object itemObj : catalogItems) {
            ...
            PluginCatalogContributionEntity entity = catalogContributionRepository
                    .findByPluginId(plugin.getId())
                    .stream()
            ...
            entity.setConfigSchema(item.containsKey("configSchema")
                    ? (Map<String, Object>) item.get("configSchema") : Map.of());
            entity.setStatus(initialStatus);
            catalogContributionRepository.save(entity);
            log.info("Registered catalog contribution '{}' for plugin {}:{}", name, plugin.getName(), plugin.getVersion());
        }
    }
    ...
    private com.scal.muxon.api.enums.CatalogItemType parseCatalogItemType(String type) {
        return switch (type == null ? "" : type.toLowerCase()) {
            case "runtime-offering" -> com.scal.muxon.api.enums.CatalogItemType.RUNTIME_OFFERING;
            case "stack-blueprint"  -> com.scal.muxon.api.enums.CatalogItemType.STACK_BLUEPRINT;
            default                 -> com.scal.muxon.api.enums.CatalogItemType.SERVICE_OFFERING;
        };
    }
}
