...
import com.scal.muxon.api.DatacentersApi;
import com.scal.muxon.api.model.*;
import com.scal.muxon.db.model.DatacenterNetworkCapabilitiesEntity;
...
import com.scal.muxon.services.DatacenterNetworkCapabilitiesService;
import com.scal.muxon.services.DatacentersService;
import com.scal.muxon.services.FabricNetworksService;
import com.scal.muxon.services.NetworkEdgeNodesService;
import com.scal.muxon.services.PublicIpPoolsService;
...
public class DatacentersController implements DatacentersApi {
    ...
    public ResponseEntity<List<FabricNetwork>> listFabricNetworks(UUID datacenterId) {
        return ResponseEntity.ok(fabricNetworksService.listByDatacenter(datacenterId).stream()
                .map(this::toFabricNetwork).toList());
    }

    @Override
    public ResponseEntity<FabricNetwork> createFabricNetwork(UUID datacenterId, FabricNetworkCreate create) {
        FabricNetworkEntity entity = fabricNetworksService.create(
                datacenterId,
                create.getName(),
    ...
    public ResponseEntity<List<PublicIpPool>> listPublicIpPools(UUID datacenterId) {
        return ResponseEntity.ok(publicIpPoolsService.listByDatacenter(datacenterId).stream()
                .map(this::toPublicIpPool).toList());
    }

    @Override
    public ResponseEntity<PublicIpPool> createPublicIpPool(UUID datacenterId, PublicIpPoolCreate create) {
        PublicIpPoolEntity entity = publicIpPoolsService.create(
    ...
    public ResponseEntity<List<NetworkEdgeNode>> listNetworkEdgeNodes(UUID datacenterId) {
        return ResponseEntity.ok(edgeNodesService.listByDatacenter(datacenterId).stream()
                .map(this::toNetworkEdgeNode).toList());
    }

    @Override
    public ResponseEntity<NetworkEdgeNode> createNetworkEdgeNode(UUID datacenterId, NetworkEdgeNodeCreate create) {
        NetworkEdgeNodeEntity entity = edgeNodesService.register(
                datacenterId,
                create.getName(),
                create.getHost(),
    ...
    public ResponseEntity<NetworkEdgeNode> updateNetworkEdgeNode(
        ...
        NetworkEdgeNodeEntity.NetworkEdgeNodeType type = create.getType() != null
                ? NetworkEdgeNodeEntity.NetworkEdgeNodeType.valueOf(create.getType().name())
                : null;
        NetworkEdgeNodeEntity entity = edgeNodesService.update(
                nodeId, create.getName(), create.getHost(), type, null);
    ...
    @Override
    public ResponseEntity<DatacenterNetworkCapabilities> getDatacenterNetworkCapabilities(UUID datacenterId) {
    ...
    public ResponseEntity<DatacenterNetworkCapabilities> updateDatacenterNetworkCapabilities(
        ...
        DatacenterNetworkCapabilitiesEntity entity = capabilitiesService.update(
                datacenterId,
