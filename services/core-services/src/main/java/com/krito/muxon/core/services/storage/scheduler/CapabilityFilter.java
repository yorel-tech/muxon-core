package com.krito.muxon.core.services.storage.scheduler;
 ...
 * <p>
 * Eliminates storage that doesn't meet the required capabilities and constraints.
 * </p>
 */
@Component
public class CapabilityFilter {
    ...
    public List<ProviderStorageEntity> filter(
        ...
        for (ProviderStorageEntity storage : candidates) {
            if (matchesCapabilities(storage, context) && 
                meetsConstraints(storage, context)) {
                filtered.add(storage);
            }
        }
        
        log.debug("Filtered {} candidates to {} matching storage entries", 
    ...
    private boolean matchesCapabilities(ProviderStorageEntity storage, SchedulerContext context) {
        ...
        for (Map.Entry<String, Object> entry : required.entrySet()) {
            ...
            if (!matchesValue(requiredValue, actualValue)) {
                log.debug("Storage {} does not match capability {}: required={}, actual={}", 
                    storage.getName(), capability, requiredValue, actualValue);
                return false;
            }
        }
    ...
    private boolean meetsConstraints(ProviderStorageEntity storage, SchedulerContext context) {
        ...
        if (constraints.containsKey("min_iops")) {
            ...
            if (actualIops < minIops) {
                log.debug("Storage {} does not meet min_iops: required={}, actual={}", 
                    storage.getName(), minIops, actualIops);
                return false;
            }
        }
        ...
        if (freeGbObj != null) {
            ...
            if (freeGb < sizeGb) {
                log.debug("Storage {} does not have enough capacity: required={}GB, available={}GB", 
                    storage.getName(), sizeGb, freeGb);
                return false;
            }
        }
