...
public interface VmRepository extends JpaRepository<VmEntity, UUID> {
  ...
  @Query(
      "SELECT v FROM VmEntity v WHERE v.tenantDatacenterGrantId IN "
  ...
  @Query(
      "SELECT v FROM VmEntity v WHERE v.id = :vmId AND v.tenantDatacenterGrantId IN "
          + "(SELECT g.id FROM TenantDatacenterGrantEntity g WHERE g.tenant.id = :tenantId)")
  Optional<VmEntity> findByIdAndTenantId(
      @Param("vmId") UUID vmId, @Param("tenantId") UUID tenantId);
  ...
  @Query(
      "SELECT v FROM VmEntity v WHERE v.status = :status AND v.tenantDatacenterGrantId = :tenantDatacenterGrantId ORDER BY v.createdAt DESC")
  Page<VmEntity> findByStatusAndTenantDatacenterGrantId(
      VmStatus status, UUID tenantDatacenterGrantId, Pageable pageable);

  /** Find VM by ID */
  Optional<VmEntity> findById(UUID id);

  /** Find VM by name and tenant */
  Optional<VmEntity> findByTenantDatacenterGrantIdAndName(
      UUID tenantDatacenterGrantId, String name);

  /** Find VMs by tags */
  @Query(
      value =
          "SELECT v FROM VmEntity v WHERE v.tenantDatacenterGrantId = :tenantDatacenterGrantId AND :tag = ANY(v.tags) ORDER BY v.createdAt DESC",
      nativeQuery = true)
  Page<VmEntity> findByTenantDatacenterGrantIdAndTag(
      UUID tenantDatacenterGrantId, String tag, Pageable pageable);
  ...
  @Query(
      "SELECT COUNT(v) FROM VmEntity v WHERE v.tenantDatacenterGrantId IN "
          + "(SELECT g.id FROM TenantDatacenterGrantEntity g WHERE g.tenant.id = :tenantId)")
