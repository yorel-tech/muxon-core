 ...
 *
 * <p>Volumes are the primary storage abstraction in Muxon, providing persistent block storage that
 * can be attached to VMs, pods, or containers. Each volume belongs to a workspace and is backed by
 * a specific storage provider.
 *
 * <p>Lifecycle states: creating → available → attaching → in_use → detaching → available → deleting
 * → deleted
 */
@Entity
@Table(
