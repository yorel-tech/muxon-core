...
import com.yorel.muxon.api.enums.EventType;
import com.yorel.muxon.db.model.EntityEventEntity;
...
public interface EntityEventRepository extends JpaRepository<EntityEventEntity, UUID> {
  ...
  @Query(
      "SELECT e FROM EntityEventEntity e WHERE e.createdAt > :afterTime ORDER BY e.createdAt DESC")
  List<EntityEventEntity> findEventsAfter(@Param("afterTime") Instant afterTime);

  /** Find events created within a time range */
  @Query(
      "SELECT e FROM EntityEventEntity e WHERE e.createdAt BETWEEN :startTime AND :endTime ORDER BY e.createdAt DESC")
