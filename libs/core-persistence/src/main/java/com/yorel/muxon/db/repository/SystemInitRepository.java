...
import com.yorel.muxon.db.model.SystemInitEntity;
...
/**
 * Repository for managing system initialization status. Provides methods to query and update
 * bootstrap status in the system_init table.
 */
public interface SystemInitRepository extends JpaRepository<SystemInitEntity, String> {
  ...
  @Query(
      value =
          "UPDATE system_init SET value = :systemStatusName, system_status = cast(:systemStatusName as bootstrap_status), updated_at = :updatedAt WHERE primary_key = :primaryKey",
      nativeQuery = true)
  int updateSystemStatus(
      @Param("primaryKey") String primaryKey,
      @Param("systemStatus") BootstrapStatus systemStatus,
  ...
  default boolean isBootstrapCompleted() {
    ...
    return status
        .map(SystemInitEntity::getSystemStatus)
        .filter(BootstrapStatus.READY::equals)
        .isPresent();
  }
  ...
  /**
   * Set the bootstrap status to BOOTSTRAPPED. This is called after muxon-initializer has inserted
   * the initial data.
   *
   * @return number of rows updated
   */
  default int markAsBootstrapped() {
    return updateSystemStatus(
        Constants.BOOTSTRAP_STATUS_KEY,
  ...
  /**
   * Set the bootstrap status to READY. This is called after all required setup steps are completed.
   *
   * @return number of rows updated
   */
