...
public class TaskRouter {
  ...
  /**
   * Routes a PLUGIN_RESOURCE command through the plugin framework dispatch path. The resource kind
   * is resolved from the command payload. If the plugin framework is disabled, fails fast with a
   * clear error.
   */
  private void routePluginResource(CommandMessage command) {
    ...
    if ("vm".equalsIgnoreCase(kind)) {
      log.error(
          "'vm' kind must be dispatched as EntityType.VM, not PLUGIN_RESOURCE. Command: {}",
          command.id());
      return;
    }
    ...
    if (target.isEmpty()) {
      log.error(
          "No active plugin registered for kind '{}' — rejecting command {}", kind, command.id());
      return;
    }
