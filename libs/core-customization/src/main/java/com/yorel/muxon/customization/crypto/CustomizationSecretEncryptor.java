 ...
 *
 * <p>The master key is injected at startup (typically from an env var or secrets manager). The key
  ...
  public CustomizationSecretEncryptor(String base64Key) {
    ...
    if (keyBytes.length != 32) {
      throw new IllegalArgumentException(
          "Customization encryption key must be exactly 32 bytes (256-bit AES); got "
              + keyBytes.length);
    }
