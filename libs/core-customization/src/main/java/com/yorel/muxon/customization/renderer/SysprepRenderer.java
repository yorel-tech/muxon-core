...
import com.yorel.muxon.customization.model.NicCustomizationSpec;
...
import com.yorel.muxon.customization.model.WindowsDomainJoin;
 ...
 *
 * <p>The file is placed on an ISO with volume label {@code UNATTEND}. Windows Setup automatically
 * picks it up from removable media at OOBE.
 *
 * <p><strong>Passes generated:</strong>
 *
 * <ul>
 *   <li>{@code specialize} – ComputerName, TimeZone, Product Key, TCP/IP, domain/workgroup join,
 *       pre-scripts via {@code RunSynchronousCommand}
 *   <li>{@code oobeSystem} – OOBE settings, AdminPassword, AutoLogon, FirstLogonCommands
 *       (post-scripts)
 * </ul>
 */
public class SysprepRenderer implements CustomizationRenderer {
  ...
  private String renderAutounattend(String vmName, VmCustomizationSpec spec) {
    ...
    if (win != null && win.getProductKey() != null && !win.getProductKey().isBlank()) {
      xml.append("      <ProductKey>")
          .append(xmlEsc(win.getProductKey()))
          .append("</ProductKey>\n");
    }
      ...
      for (int i = 0; i < spec.getNics().size(); i++) {
        ...
        if (nic.isStatic()) {
          ...
          xml.append("            <IpAddress wcm:action=\"add\" wcm:keyValue=\"1\">")
              .append(xmlEsc(nic.getIpAddress()))
              .append("/")
              .append(nic.getPrefix())
              .append("</IpAddress>\n");
          xml.append("          </UnicastIpAddresses>\n");
          if (nic.getGateway() != null && !nic.getGateway().isBlank()) {
            ...
            xml.append("              <NextHopAddress>")
                .append(xmlEsc(nic.getGateway()))
                .append("</NextHopAddress>\n");
            xml.append("            </Route>\n");
            xml.append("          </Routes>\n");
          }
        } else {
    ...
    if (win != null && win.getJoinDomain() != null) {
      ...
      if (dj.getUsername() != null) {
        ...
        if (dj.getPassword() != null) {
          xml.append("          <Password>")
              .append(xmlEsc(dj.getPassword()))
              .append("</Password>\n");
        }
        xml.append("        </Credentials>\n");
      }
      if (dj.getOu() != null && !dj.getOu().isBlank()) {
        xml.append("        <MachineObjectOU>")
            .append(xmlEsc(dj.getOu()))
            .append("</MachineObjectOU>\n");
      }
      xml.append("      </Identification>\n");
      xml.append("    </component>\n");
    } else {
    ...
    if (win != null && win.getResolvedPreScripts() != null) {
      preScripts.addAll(win.getResolvedPreScripts());
    }
    ...
    if (win != null && win.getFirstLogonCommands() != null) {
      postCmds.addAll(win.getFirstLogonCommands());
    }
    if (win != null && win.getResolvedPostScripts() != null) {
      postCmds.addAll(win.getResolvedPostScripts());
    }
    if (!postCmds.isEmpty()) {
      ...
      for (int i = 0; i < postCmds.size(); i++) {
        ...
        xml.append("          <Order>").append(i + 1).append("</Order>\n");
        xml.append("          <CommandLine>")
            .append(xmlEsc(postCmds.get(i)))
            .append("</CommandLine>\n");
    ...
    // Locale / InputLocale
    if (win != null) {
      xml.append("      <UILanguage>")
          .append(xmlEsc(win.getUiLanguage()))
          .append("</UILanguage>\n");
      xml.append("      <SystemLocale>")
          .append(xmlEsc(win.getSystemLocale()))
          .append("</SystemLocale>\n");
      xml.append("      <UserLocale>")
          .append(xmlEsc(win.getUserLocale()))
          .append("</UserLocale>\n");
      xml.append("      <InputLocale>")
          .append(xmlEsc(win.getInputLocale()))
          .append("</InputLocale>\n");
    }
  ...
  /**
   * Microsoft's AdminPassword encoding: Base64(password + "AdministratorPassword"). This is
   * obfuscation, not encryption; the seed ISO is short-lived and stored at 0600.
   */
  private static String encodeAdminPassword(String password) {
    String toEncode = password + "AdministratorPassword";
    return Base64.getEncoder()
        .encodeToString(toEncode.getBytes(java.nio.charset.StandardCharsets.UTF_16LE));
  }
