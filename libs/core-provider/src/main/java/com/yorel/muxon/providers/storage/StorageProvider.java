 ...
 *
 * <p>This SPI defines the contract between Muxon's storage orchestration layer and infrastructure
 * providers (Libvirt, Proxmox, cloud providers). Implementations handle provider-specific storage
 * operations for volumes, snapshots, and object storage buckets.
 *
...
public interface StorageProvider {
   ...
   *
   * <p>The provider should allocate storage according to the request parameters and return the
   * provider-specific volume identifier. The volume will be in "creating" status initially and
   * should transition to "available" once ready.
   *
   ...
   *
   * <p>The provider should release all storage resources associated with the volume. This operation
   * should fail if the volume is currently attached.
   *
   ...
   *
   * <p>The new size must be larger than the current size. The provider should expand the volume
   * while preserving existing data. Some providers may require the volume to be detached for this
   * operation.
   *
   * @param volumeId provider-specific volume identifier
   ...
   *
   * <p>The provider should make the volume accessible to the specified resource at the given device
   * path. The volume should transition to "in_use" status.
   *
   ...
   *
   * <p>The provider should safely disconnect the volume from the resource. The volume should
   * transition to "available" status after detachment.
   *
   * @param volumeId provider-specific volume identifier
   ...
   *
   * <p>The provider should create a consistent snapshot of the volume's current state. The snapshot
   * can be used later for restore or clone operations.
   *
   ...
   * Delete a snapshot.
   *
   * <p>The provider should release storage resources associated with the snapshot. This operation
   * should fail for immutable snapshots within their retention period.
   *
   ...
   *
   * <p>Creates a new volume with the contents of the specified snapshot. The new volume will have
   * the same size as the snapshot.
   *
   ...
   *
   * <p>Creates a new volume that initially shares storage with the snapshot but diverges as writes
   * occur. This is more efficient than full restore for providers that support CoW (e.g., Ceph RBD,
   * ZFS).
   *
   ...
   *
   * <p>The provider should create an S3-compatible bucket with the specified configuration. Returns
   ...
   *
   * <p>The provider should delete the bucket and all its contents. This operation should fail if
   * the bucket is not empty (provider-dependent).
   *
   ...
   *
   * <p>Returns information about supported storage classes, backend types, available capacity, and
   * feature support (encryption, snapshots, etc.).
   *
