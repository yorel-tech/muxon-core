...
/**
 * Transport-agnostic port for sending and consuming commands. Implementations may use a database
 * table (OSS) or a message bus provided by enterprise extensions.
 */
public interface CommandQueue {
  ...
  /**
   * Poll for pending commands of the given entity type. Implementation should apply
   * visibility/claim semantics (e.g. set PROCESSING) so the same entry is not processed by another
   * worker. Caller must eventually call markCompleted or markFailed.
   *
  ...
  /**
   * Mark a command completed and persist the final payload in one transaction. Use this when the
   * API polls on {@code COMPLETED} and reads {@code payload} (e.g. console resolve); otherwise a
   * separate payload update plus {@link #markCompleted(UUID)} can commit status before payload is
   * visible.
   */
  ...
  /**
   * Get stalled (old pending/processing) entries for monitoring or reset. Optional: implementations
  ...
  /**
   * Reset stalled entries back to PENDING so they can be retried. Optional: implementations may
   * throw UnsupportedOperationException if not applicable.
   *
