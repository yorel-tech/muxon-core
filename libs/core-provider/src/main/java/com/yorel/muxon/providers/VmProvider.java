...
/**
 * Core provider interface for VM orchestration. This SPI defines the contract between orchestrator
...
public interface VmProvider {
  ...
  default CompletableFuture<VmConsoleConnectionInfo> getConsoleConnection(
      VmConsoleRequest request) {
    return CompletableFuture.failedFuture(
        new UnsupportedOperationException("Console is not supported for this provider"));
  }

  /**
   * Query the QEMU guest agent channel for the VM's current IP addresses, hostname, and (Linux
   * only) the cloud-init status JSON.
   *
   * <p>Returns {@link GuestAgentInfo#unreachable()} when the guest agent has not yet started rather
   * than throwing, so callers can poll until the agent is available.
   *
   ...
   */
  default CompletableFuture<VmOperationResult> detachCustomizationSeed(
