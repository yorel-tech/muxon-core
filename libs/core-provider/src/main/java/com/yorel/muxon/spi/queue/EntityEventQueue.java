 ...
 *
 * <p>OSS: backed by the {@code orchestrator_queue} table (ENTITY_EVENT category) in {@code
 * DbEntityEventQueue}.
 *
...
public interface EntityEventQueue {
  ...
  /**
   * Publish an entity state change event (called by the worker after a successful provider
   * operation).
   *
  ...
  /**
   * Poll for pending entity events. Implementation must claim entries (mark PROCESSING) so the same
