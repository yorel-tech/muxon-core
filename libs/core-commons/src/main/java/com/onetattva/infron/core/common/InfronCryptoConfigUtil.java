 ...
 */
public final class InfronCryptoConfigUtil {

    private static final String INFRON_PASSPHRASE_FILE = "/run/secrets/infron-passphrase";

    private InfronCryptoConfigUtil() {
    }

    /**
     * Initialize EncryptionUtil key using instance details and passphrase.
     *
     * @param instanceName infron.instanceName (e.g. "local")
     * @param instanceId   infron.instanceId (e.g. 19)
     */
    ...
    private static String readPassphrase() {
        try {
            return Files.readString(Paths.get(INFRON_PASSPHRASE_FILE)).trim();
        } catch (IOException e) {
            String envPassphrase = System.getenv("INFRON_PASSPHRASE");
            if (envPassphrase == null || envPassphrase.isEmpty()) {
                throw new IllegalStateException("Passphrase not configured: expected file " +
                        INFRON_PASSPHRASE_FILE + " or INFRON_PASSPHRASE env var");
            }
