...
/**
 * Relative paths for content artifacts on Proxmox/KVM storage pools: {@code
 * muxon-{instance}-{id}/content-libraries/{libraryId}/{itemId}/{filename}}.
 */
public final class ContentLibraryProviderPaths {
  ...
  public static String relativePath(
      String instanceSegment, UUID libraryId, UUID itemId, String safeFilename) {
    return instanceSegment + "/content-libraries/" + libraryId + "/" + itemId + "/" + safeFilename;
  }

  /** Full relative path under the storage pool root from raw config and item identity. */
  public static String buildRelativePath(
      String rawInstanceName, int instanceId, UUID libraryId, UUID itemId, String rawFilename) {
