...
# kcadm helper script to create roles, users, clients and audience mappers for muxon-dev
# Usage:
#   KC_URL=http://localhost:8080/auth KC_ADMIN_USER=admin KC_ADMIN_PASS=admin ./kcadm-create-muxon.sh
#
# Requirements:
...
USERS=(
  "admin@muxon.dev:Admin123!:admin"
  "dev@muxon.dev:Dev123!:member"
  "tenantadmin@muxon.dev:Tenant123!:tenantadmin"
  "projectadmin@muxon.dev:Project123!:projectadmin"
  "workload-operator@muxon.dev:Operator123!:workload-operator"
  "workload-user@muxon.dev:User123!:workload-user"
)
...
CLIENTS=(
  'muxon-web|true|false|["http://localhost:4000/*"]|["http://localhost:4000"]'
  'muxon-api|false|true|[]|[]'
  'muxon-test|true|false|["http://localhost:8081/*"]|["http://localhost:8081"]'
)

# Audience target (the API client id/name to be added to aud)
AUD_TARGET="muxon-api"
...
# Create clients if missing and add audience mappers for muxon-web and muxon-test
for c in "${CLIENTS[@]}"; do
  ...
  # If client is muxon-web or muxon-test, add audience mapper to include AUD_TARGET in access tokens
  if [ "${clientId}" = "muxon-web" ] || [ "${clientId}" = "muxon-test" ]; then
