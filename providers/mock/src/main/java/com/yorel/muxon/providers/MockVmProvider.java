...
/**
 * Mock VM provider for testing and development. Simulates VM operations without actual
 * infrastructure.
 */
@ApplicationScoped
public class MockVmProvider implements VmProvider {
  ...
  public CompletableFuture<VmCreationResult> createVm(VmCreationRequest request) {
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          return VmCreationResult.success(vm.externalId(), vm.toVmInfo());
        });
  }
  ...
  public CompletableFuture<VmOperationResult> startVm(VmOperationRequest request) {
    ...
    if (vm.powerState() == VmPowerState.ON) {
      return CompletableFuture.completedFuture(VmOperationResult.failure("VM is already running"));
    }
  ...
  public CompletableFuture<VmOperationResult> stopVm(VmOperationRequest request) {
    ...
    if (vm.powerState() == VmPowerState.OFF) {
      return CompletableFuture.completedFuture(VmOperationResult.failure("VM is already stopped"));
    }
  ...
  public CompletableFuture<VmOperationResult> resumeVm(VmOperationRequest request) {
    ...
    if (vm.powerState() != VmPowerState.SUSPENDED) {
      return CompletableFuture.completedFuture(VmOperationResult.failure("VM is not suspended"));
    }
  ...
  public CompletableFuture<List<VmInfo>> listVms(VmListRequest request) {
    ...
    return CompletableFuture.completedFuture(vms.values().stream().map(MockVm::toVmInfo).toList());
  }
  ...
  public CompletableFuture<ValidationResult> validateVmSpec(String spec) {
    ...
    ValidationResult result =
        ValidationResult.builder().valid(errors.isEmpty()).errors(errors).build();
  ...
  public CompletableFuture<VmTemplateExportResult> cloneVmAsTemplate(
    ...
    return CompletableFuture.completedFuture(
        VmTemplateExportResult.success(
            "/mock/" + request.destinationRelativePath(), 1024L, Map.of("mock", "true")));
  }

  @Override
  public CompletableFuture<VmConsoleConnectionInfo> getConsoleConnection(VmConsoleRequest request) {
    // Deterministic loopback target for integration tests; no real VNC listener unless provided
    // externally.
    return CompletableFuture.completedFuture(
