...
import com.onetattva.infron.core.providers.storage.*;
 ...
 * <p>
 * Ceph RBD (RADOS Block Device) provides distributed block storage with:
 * <ul>
 *   <li>Native snapshot support</li>
 *   <li>Copy-on-write clones</li>
 *   <li>Thin provisioning</li>
 *   <li>Encryption support</li>
 * </ul>
 * </p>
 * <p>
 * Volume IDs follow the format: ceph-rbd/{pool}/{volume-name}
 * Snapshot IDs follow the format: ceph-rbd/{pool}/{volume-name}@{snapshot-name}
 * </p>
 */
public class LibvirtCephRbdBackend implements LibvirtStorageBackend {
    ...
    public VolumeAttachmentResult attachVolume(
        ...
        try {
            ...
            log.info("Attached Ceph RBD volume: {} to resource: {} at device: {}",
    ...
    public SnapshotCreationResult createSnapshot(
            ...
            LibvirtStorageProvider.VolumeMetadata metadata) {
        try {
            String snapshotId = String.format("ceph-rbd/%s/%s@%s",
                metadata.poolName(), metadata.volumeName(), request.name());

            String command = String.format("rbd snap create %s/%s@%s",
                metadata.poolName(), metadata.volumeName(), request.name());
    ...
    public SnapshotOperationResult deleteSnapshot(
        ...
        try {
            String command = String.format("rbd snap rm %s/%s@%s",
    ...
    public VolumeCreationResult restoreFromSnapshot(
        ...
        try {
            ...
            String command = String.format("rbd clone %s/%s@%s %s/%s",
                metadata.poolName(), metadata.volumeName(), metadata.snapshotName(),
    ...
    public List<SnapshotInfo> listSnapshots(
        ...
        try {
            String command = String.format("rbd snap ls %s/%s --format json",
    ...
    private String buildVolumeXml(String name, long sizeBytes, boolean encrypted, boolean thinProvisioned) {
