 ...
 *
 * <p>Connection pooling improves performance by reusing connections and ensures proper cleanup of
 * resources.
 */
public class LibvirtConnectionManager {
  ...
  /**
   * Gets or creates a connection to libvirtd. Connections are pooled per thread for efficiency.
   *
  ...
  /**
   * Closes all open connections to libvirtd. Should be called when provider is being shut down or
   * reloaded.
   */
  public void closeAll() {
    ...
    for (Map.Entry<String, Connect> entry : connectionPool.entrySet()) {
      try {
        ...
        logger.error("Error closing Libvirt connection for URI: {}: {}", uri, e.getMessage());
      }
    }
