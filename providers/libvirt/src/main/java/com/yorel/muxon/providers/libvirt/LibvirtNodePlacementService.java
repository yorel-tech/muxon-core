...
public class LibvirtNodePlacementService {
  ...
  public Optional<NodeEntity> selectNodeForPlacement(
      List<NodeEntity> nodes, int requiredCpuCores, int requiredMemMb) {
    if (nodes == null || nodes.isEmpty()) {
      logger.warn("No nodes available for placement");
      return Optional.empty();
    }

    List<NodeEntity> eligibleNodes =
        nodes.stream()
    ...
    if (eligibleNodes.isEmpty()) {
      logger.warn(
          "No eligible nodes found with required resources (CPU: {}, Memory: {} MB)",
          requiredCpuCores,
          requiredMemMb);
      return Optional.empty();
    }
    ...
    if (selectedNode != null) {
      logger.info(
          "Selected node {} for placement (CPU: {}, Memory: {} MB)",
  ...
  private boolean hasEnoughResources(NodeEntity node, int requiredCpuCores, int requiredMemMb) {
    ...
    Integer availableMem =
        resources.getMemory() != null ? resources.getMemory().getAvailableMb() : null;
    ...
    if (!hasEnough) {
      logger.debug(
          "Node {} does not have enough resources. Available: CPU={}, Mem={} MB. Required: CPU={}, Mem={} MB",
          node.getName(),
          availableCpu,
          availableMem,
          requiredCpuCores,
  ...
  private double calculateNodeLoad(NodeEntity node) {
    ...
    logger.debug(
        "Node {} load: {:.2f} (CPU usage: {:.2f}, Memory usage: {:.2f})",
        node.getName(),
        load,
        cpuUsageRatio,
        memUsageRatio);
  ...
  public Optional<NodeEntity> selectLeastLoadedNode(List<NodeEntity> nodes) {
    ...
    List<NodeEntity> activeNodes =
        nodes.stream().filter(node -> node.getStatus() == Node.StatusEnum.READY).toList();
    ...
    return activeNodes.stream().min(Comparator.comparingDouble(this::calculateNodeLoad));
  }
}
