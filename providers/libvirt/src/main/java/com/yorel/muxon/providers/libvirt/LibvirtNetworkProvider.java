 ...
 *
 * <ul>
 *   <li>Creates libvirt virtual networks (one per subnet) via XML definition
 *   <li>Resolves NIC attachments to bridge or network names based on fabric_network type
 *   <li>Applies nftables rules for security group enforcement at vNIC level
 * </ul>
 */
public class LibvirtNetworkProvider implements NetworkProvider {
  ...
  public LibvirtNetworkProvider(
      UUID providerId, LibvirtMultiNodeConnectionManager connectionManager) {
  ...
  public CompletableFuture<String> createSubnetSegment(SubnetSpec spec) {
    return CompletableFuture.supplyAsync(
        () -> {
          String networkName = "muxon-subnet-" + spec.subnetId();
          try {
            // Get a connection to any available hypervisor node
            Connect conn = connectionManager.getAnyConnection();
            if (conn == null) {
              logger.warn(
                  "No libvirt connection available; subnet {} created with stub handle",
                  spec.subnetId());
              return networkName;
            }
            ...
            try {
              ...
              logger.warn(
                  "Libvirt network creation for {} returned: {}", networkName, e.getMessage());
            }
            return networkName;
          } catch (Exception e) {
            logger.error(
                "Failed to create libvirt network segment for subnet {}: {}",
                spec.subnetId(),
                e.getMessage());
        ...
        () -> {
          try {
            ...
            try {
              ...
              logger.warn(
                  "Libvirt network {} not found or already deleted: {}",
                  providerHandle,
                  e.getMessage());
            }
          } catch (Exception e) {
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          if ("BRIDGE".equalsIgnoreCase(fabricType)) {
            String bridgeName =
                ref.fabricNetworkExternalId() != null
            ...
            String networkName =
                ref.providerHandle() != null
                    ? ref.providerHandle()
                    : "muxon-subnet-" + ref.subnetId();
  ...
  @Override
  public CompletableFuture<Void> applyNicPolicy(
    ...
    return CompletableFuture.runAsync(
        () -> {
          ...
          logger.info(
              "Applying {} SG rules to VM {} NIC {}",
              securityGroupRules.size(),
              vmExternalId,
              nicMacAddress);
  ...
  private String buildNetworkXml(String networkName, SubnetSpec spec) {
    ...
    return "<network>\n"
        ...
        + "  <forward mode='nat'/>\n"
        + "  <bridge name='"
        + networkName.replace("muxon-subnet-", "br-")
        + "' stp='on' delay='0'/>\n"
        + "  <ip address='"
        + gateway
        + "' netmask='"
