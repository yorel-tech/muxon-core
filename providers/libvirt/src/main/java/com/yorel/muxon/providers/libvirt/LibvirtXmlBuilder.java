 ...
 *
 * <p>Converts Muxon VM specifications to Libvirt-compatible XML format for domain creation.
 */
public class LibvirtXmlBuilder {
  ...
  /**
   * Builds complete Libvirt domain XML for VM creation with an explicit disk path. Use this when
   * the disk image is created via a storage pool (e.g. StorageVol.getPath()).
   *
   * @param vmId The VM ID
   * @param spec The VM specification JSON string
  ...
  /**
   * Builds domain XML with optional ISO CD-ROM devices and optional customization seed. {@link
   * IsoAttachment#deviceName()} selects the guest target dev (e.g. sdc); when null, assigns sdc,
   * sdd, ...
   */
  public static String buildDomainXml(
      UUID vmId, String spec, String diskPath, List<IsoAttachment> isoAttachments) {
   ...
   *
   * @param customizationSeed When non-null, appends a second CD-ROM at {@code sdb} with the seed
   *     ISO.
  ...
  public static String buildDomainXml(
    ...
    return buildDomainXml(
        vmId, spec, diskPath, isoAttachments, customizationSeed, enableGuestAgent, null);
  }

  /**
   * Full builder with subnet-resolved NIC attachment. When {@code resolvedNic} is non-null, uses
   * the resolved network/bridge instead of 'default'.
   *
   * @param resolvedNic Resolved NIC attachment from NetworkProvider.resolveNicAttachment(); falls
   *     back to {@code <source network='default'/>} if null (legacy path for VMs without subnet_id)
   */
  public static String buildDomainXml(
      UUID vmId,
      String spec,
    ...
    if (customizationSeed != null) {
      ...
      xml.append("      <source file='")
          .append(escapeXmlAttr(customizationSeed.isoPath()))
          .append("'/>\n");
    ...
    if (resolvedNic != null
        && resolvedNic.attachmentType() == ResolvedNicAttachment.AttachmentType.BRIDGE) {
      xml.append("    <interface type='bridge'>\n");
      xml.append("      <source bridge='")
          .append(escapeXmlAttr(resolvedNic.bridgeName()))
          .append("'/>\n");
      xml.append("      <model type='virtio'/>\n");
      xml.append("    </interface>\n");
    } else {
      String networkName =
          (resolvedNic != null && resolvedNic.networkName() != null)
              ? resolvedNic.networkName()
              : "default";
