...
public class LibvirtMultiNodeConnectionManager {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(LibvirtMultiNodeConnectionManager.class);
  ...
  public List<NodeEntity> getActiveNodes() {
    ...
    return allNodes.stream().filter(node -> node.getStatus() == Node.StatusEnum.READY).toList();
  }
  ...
  /**
   * Get any available connection from active nodes. Returns the first active node connection, or
   * null if no active nodes are available.
   *
  ...
  private LibvirtConnectionManager getOrCreateConnectionManager(UUID nodeId) {
    return nodeConnections.computeIfAbsent(
        nodeId,
        id -> {
  ...
  private String buildLibvirtUriFromNode(NodeEntity node) {
    try {
      ...
      logger.error(
          "Failed to build Libvirt URI from node credentials for node {}: {}",
          node.getId(),
          e.getMessage(),
          e);
