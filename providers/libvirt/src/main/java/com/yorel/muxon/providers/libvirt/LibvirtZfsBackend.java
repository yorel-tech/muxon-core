 ...
 *
 * <p>ZFS provides advanced storage features:
 *
 * <ul>
 *   <li>Native copy-on-write snapshots
 *   <li>Efficient cloning
 *   <li>Compression and deduplication
 *   <li>Data integrity verification
 * </ul>
 *
 * <p>Volume IDs follow the format: zfs/{pool}/{dataset} Snapshot IDs follow the format:
...
public class LibvirtZfsBackend implements LibvirtStorageBackend {
  ...
  public VolumeCreationResult createVolume(
    ...
    try {
      ...
      String volumeXml =
          buildVolumeXml(request.name(), request.sizeBytes(), request.encrypted(), config);
  ...
  public VolumeOperationResult resizeVolume(
    ...
    try {
      String command =
          String.format(
              "zfs set volsize=%d %s/%s", newSizeBytes, metadata.poolName(), metadata.volumeName());
  ...
  public VolumeAttachmentResult attachVolume(
    ...
    try {
      ...
      log.info(
          "Attached ZFS volume: {} to resource: {} at device: {}",
          request.volumeId(),
          request.resourceId(),
          device);
  ...
  public SnapshotCreationResult createSnapshot(
    ...
    try {
      String snapshotId =
          String.format("zfs/%s/%s@%s", metadata.poolName(), metadata.volumeName(), request.name());

      String command =
          String.format(
              "zfs snapshot %s/%s@%s", metadata.poolName(), metadata.volumeName(), request.name());
  ...
  public VolumeCreationResult restoreFromSnapshot(
    ...
    try {
      ...
      String providerVolumeId =
          String.format("zfs/%s/%s", metadata.poolName(), metadata.volumeName());

      log.info("Restored volume from snapshot: {}", request.snapshotId());

      return VolumeCreationResult.success(
          metadata.volumeName(), providerVolumeId, providerVolumeId, 0L);
  ...
  public List<SnapshotInfo> listSnapshots(
    ...
    try {
      String command =
          String.format(
              "zfs list -t snapshot -o name -H | grep %s/%s@",
  ...
  private String buildVolumeXml(
      String name, long sizeBytes, boolean encrypted, Map<String, Object> config) {
