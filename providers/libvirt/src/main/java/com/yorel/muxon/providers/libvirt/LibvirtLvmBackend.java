 ...
 *
 * <p>LVM provides block storage with:
 *
 * <ul>
 *   <li>Thin provisioning support via thin pools
 *   <li>Snapshot support (copy-on-write)
 *   <li>Volume resizing
 *   <li>Encryption via LUKS
 * </ul>
 *
 * <p>Volume IDs follow the format: lvm/{vg-name}/{lv-name} Snapshot IDs follow the format:
 * lvm/{vg-name}/{lv-name}@{snapshot-name}
 */
public class LibvirtLvmBackend implements LibvirtStorageBackend {
    ...
    try {
      ...
      if (volumeGroup == null) {
        return VolumeCreationResult.failure(
            request.name(), "Volume group not configured in storage mapping");
      }

      StoragePool pool = connection.storagePoolLookupByName(volumeGroup);
      if (pool == null) {
        return VolumeCreationResult.failure(
            request.name(), "Storage pool not found: " + volumeGroup);
      }
  ...
  public VolumeAttachmentResult attachVolume(
    ...
    try {
      ...
      log.info(
          "Attached LVM volume: {} to resource: {} at device: {}",
          request.volumeId(),
          request.resourceId(),
          device);
  ...
  public SnapshotCreationResult createSnapshot(
    ...
    try {
      ...
      String snapshotId =
          String.format("lvm/%s/%s@%s", metadata.poolName(), metadata.volumeName(), request.name());

      String command =
          String.format(
              "lvcreate -s -n %s /dev/%s/%s",
  ...
  public SnapshotOperationResult deleteSnapshot(
    ...
    try {
      ...
      String command = String.format("lvremove -f /dev/%s/%s", metadata.poolName(), snapshotName);
  ...
  public VolumeCreationResult restoreFromSnapshot(
    ...
    try {
      ...
      String command =
          String.format(
              "lvcreate -s -n %s /dev/%s/%s", newVolumeName, metadata.poolName(), snapshotName);
  ...
  public List<SnapshotInfo> listSnapshots(
    ...
    try {
      String command =
          String.format(
              "lvs --noheadings -o lv_name /dev/%s | grep %s_",
