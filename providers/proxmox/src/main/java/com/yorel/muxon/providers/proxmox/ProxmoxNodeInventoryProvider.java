...
public final class ProxmoxNodeInventoryProvider {
  ...
  /**
   * Nodes plus optional Proxmox cluster name from {@code /cluster/status} (row with type {@code
   * cluster}).
   */
  ...
  private static Optional<String> readProxmoxClusterName(Proxmox proxmox) {
    try {
      ...
      log.debug(
          "Proxmox cluster status not available for cluster name (standalone or API error): {}",
          e.getMessage());
    }
    return Optional.empty();
  }
