 ...
 *
 * <p>Discovers storage configurations from Proxmox clusters and normalizes them into the common
 * capability model. This implementation uses the Proxmox API client to connect to clusters and
 * enumerate storage.
 */
public class ProxmoxStorageDiscoveryProvider implements StorageDiscoveryProvider {
  ...
  private static final Map<String, Map<String, Object>> STORAGE_TYPE_CAPABILITIES =
      Map.ofEntries(
          Map.entry(
              "rbd",
              Map.of(
  ...
  public CompletableFuture<List<DiscoveredStorage>> discoverStorage(
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          log.info(
              "[proxmox-storage] discoverStorage async task started: providerId={}", providerId);
          ...
          try {
            ...
            if (!(nodesResult instanceof List<?> nodes)) {
              log.warn(
                  "[proxmox-storage] nodes index did not return a list (got {}): providerId={}",
                  nodesResult != null ? nodesResult.getClass().getName() : "null",
                  providerId);
              return discoveredStorage;
            }

            log.info(
                "[proxmox-storage] cluster reports {} node(s): providerId={}",
                nodes.size(),
                providerId);
            ...
            for (Object nodeObj : nodes) {
              ...
              if (nodeName == null || nodeName.isEmpty()) {
                log.debug(
                    "[proxmox-storage] skipping node entry with empty name: providerId={}",
                    providerId);
                continue;
              }
              ...
              if (storageApi != null) {
                ...
                Object storageResult =
                    storageIndexApi != null ? tryInvokeMethod(storageIndexApi, "execute") : null;

                if (storageResult instanceof List<?> storageList) {
                  log.info(
                      "[proxmox-storage] node {} returned {} raw storage row(s): providerId={}",
                      nodeName,
                      storageList.size(),
                      providerId);
                  for (Object storageObj : storageList) {
                    DiscoveredStorage storage =
                        discoverStorageEntry(storageObj, nodeName, providerId, processedStorage);
                    if (storage != null) {
                  ...
                  log.warn(
                      "[proxmox-storage] node {} storage index was not a list (got {}): providerId={}",
                      nodeName,
                      storageResult != null ? storageResult.getClass().getName() : "null",
                      providerId);
                }
              } else {
                log.warn(
                    "[proxmox-storage] node {} has no getStorage API: providerId={}",
                    nodeName,
                    providerId);
              }
            }
            ...
            log.info(
                "[proxmox-storage] discovered {} unique storage entries: providerId={}",
                discoveredStorage.size(),
                providerId);

          } catch (Exception e) {
            log.error(
                "[proxmox-storage] discovery failed for provider {}: {}",
                providerId,
                e.getMessage(),
                e);
          }
  ...
  @Override
  public CompletableFuture<StorageDiscoveryTestResult> testConnection(
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          try {
            ...
            return StorageDiscoveryTestResult.failure("Failed to connect: " + e.getMessage());
          }
        });
  }
  ...
  private DiscoveredStorage discoverStorageEntry(
    ...
    try {
      ...
      Map<String, Object> capabilities =
          new HashMap<>(
              STORAGE_TYPE_CAPABILITIES.getOrDefault(
                  storageType,
                  Map.of(
                      "performance", "medium",
                      "media", "unknown",
      ...
      if (content != null && !content.isBlank()) {
        capabilities.put(
            "content_types",
            Arrays.stream(content.split(",")).map(String::trim).filter(s -> !s.isEmpty()).toList());
      }
      ...
      log.debug(
          "Discovered Proxmox storage: id={}, type={}, node={}, capabilities={}, metrics={}",
          storageId,
          storageType,
          nodeName,
          capabilities,
      ...
      return new DiscoveredStorage(
          storageId, storageId, storageType, capabilities, metrics, nodeName);
  ...
  private Proxmox createProxmoxClient(Map<String, Object> connectionInfo) {
    ...
    try {
      return Proxmox.createWithPassword(
          host, port, username, password, realm, SecurityConfig.insecure());
    } catch (Exception e) {
  ...
  private Object invokeMethodWithArg(Object target, String methodName, Object arg)
      throws Exception {
    try {
