 ...
 */
package com.yorel.muxon.tests.vm;
...
import com.yorel.muxon.api.model.OsSpec;
...
import com.yorel.muxon.tests.util.VmTestUtil;
...
/**
 * Integration tests for VM functionality. Tests VM lifecycle operations including creation,
...
public class VmTests extends BaseIntegrationTest {
  ...
  public static void setup() {
    ...
    Response datacenterResponse =
        ProviderTestUtil.createMockDatacenter(datacenterName, "Datacenter for VM tests");
  ...
  public void testUpdateVm() {
    ...
    VmUpdateRequest updateRequest =
        VmTestUtil.createVmUpdateRequest("Updated description for test VM", metadata);
  ...
  public void testVmPowerOperations() {
    ...
    if (isActive) {
      ...
      stopResponse.then().body("message", containsString("stop"));
    }
  ...
  public void testVmSuspendResumeOperations() {
    ...
    if (isActive) {
      ...
      suspendResponse.then().body("message", containsString("suspend"));

      Response resumeResponse = VmTestUtil.resumeVm(vmId);
      assertStatusCode(resumeResponse, 200);
      resumeResponse.then().body("message", containsString("resume"));
    }
  ...
  public void testVmResourceUsage() {
    ...
    if (usageResponse.getStatusCode() == 200) {
      usageResponse.then().body("cpu", notNullValue()).body("memory", notNullValue());
    }
  ...
  public void testCreateVmWithInvalidName() {
    ...
    response.then().body("error", notNullValue()).body("message", containsString("name"));
  }
  ...
  public void testCreateVmWithInvalidSpec() {
    ...
    assertStatusCode(response, 400);
    response.then().body("error", notNullValue()).body("message", containsString("cpu"));
  }
  ...
  public void testGetNonExistentVm() {
    ...
    assertStatusCode(response, 404);
    response.then().body("error", notNullValue()).body("message", containsString("VM"));
  }
