...
/**
 * Utility class for provider-related test operations. Tests provider functionality through
 * datacenter API.
 */
public class ProviderTestUtil {
  ...
  /**
   * Create a datacenter with mock provider type. Creates a provider and node cluster first, then a
   * datacenter backed by that cluster.
   */
  public static Response createMockDatacenter(String name, String description) {
    try {
      ...
      Response providerResponse =
          createProvider(
              generateUniqueProviderName(), ProviderType.LIBVIRT, "qemu:///system", Map.of());
      if (providerResponse.getStatusCode() != 201) {
        throw new RuntimeException(
            "Failed to create provider: " + providerResponse.getBody().asString());
      }
      ...
      if (clusterResponse.getStatusCode() != 201) {
        throw new RuntimeException(
            "Failed to create node cluster: " + clusterResponse.getBody().asString());
      }
  ...
  public static Response updateDatacenterSettings(
      String datacenterId, DatacenterSettings settings) {
    try {
      return given()
          .header("Authorization", "Bearer " + accessToken)
          .contentType("application/json")
  ...
  public static Response createProvider(
      String name, ProviderType type, String endpoint, Map<String, String> credentials) {
    try {
  ...
  /** List providers */
  public static Response listProviders(
  ...
  /**
   * Minimal SSH private key (Ed25519) for Libvirt provider validation tests. Used when only
   * presence of sshPrivateKey is validated; connection may still fail.
   */
  public static final String MINIMAL_SSH_PRIVATE_KEY =
