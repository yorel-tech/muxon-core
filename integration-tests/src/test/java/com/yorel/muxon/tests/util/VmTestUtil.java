...
import com.yorel.muxon.api.model.NetworkSpec;
import com.yorel.muxon.api.model.NicSpec;
...
public class VmTestUtil {
  ...
  public static Response listVms() {
    return given().header("Authorization", "Bearer " + accessToken).when().get(vmsCollectionPath());
  }

  /** Get VM by ID */
  public static Response getVm(String vmId) {
    return given().header("Authorization", "Bearer " + accessToken).when().get(vmItemPath(vmId));
  }
  ...
  public static Response deleteVm(String vmId) {
    return given().header("Authorization", "Bearer " + accessToken).when().delete(vmItemPath(vmId));
  }
  ...
  /**
   * Create a basic VM spec with default configuration aligned with the current VmSpec schema
   * (compute/storage/network/os).
   */
  ...
  /**
   * Create a VM spec with high-performance configuration aligned with the current VmSpec schema.
   */
  ...
  /** Create a VM spec with Windows configuration aligned with the current VmSpec schema. */
  ...
  public static VmUpdateRequest createVmUpdateRequest(
