...
import com.yorel.muxon.tests.RestConstants;
...
public class TenantTests extends BaseIntegrationTest {
  ...
  public static void setup() {
    ...
    // limited-admin exists in Keycloak but has no muxon role_bindings from bootstrap → no
    // tenant:manage.
    limitedAdminToken =
        getAccessToken(RestConstants.LIMITED_ADMIN_USERNAME, RestConstants.LIMITED_ADMIN_PASSWORD);
  }
  ...
  public void testListTenants() {
    ...
    response.then().body("items", notNullValue());
  }
  ...
  public void testListTenantsExcludesSystemTenant() {
    ...
    if (tenantIds != null) {
      assertFalse(
          tenantIds.contains(SYSTEM_TENANT_ID),
          "List must not include system tenant (id=" + SYSTEM_TENANT_ID + ")");
    }
    if (tenantNames != null) {
      assertFalse(
          tenantNames.stream().anyMatch("system"::equalsIgnoreCase),
  ...
  public void testGetTenant() {
    ...
    Response getResponse =
        given().header("Authorization", "Bearer " + accessToken).when().get("/tenants/" + tenantId);
  ...
  public void testCreateTenantWithoutPermission() {
    // Try to create tenant with limited-admin user (should fail due to missing tenant:manage
    // permission)
  ...
  public void testDeleteTenantWithoutManagePermission() {
    ...
    String tenantName =
        "test-tenant-delete-limited-admin-" + UUID.randomUUID().toString().substring(0, 8);
    ...
    // Try to delete tenant with limited-admin user (should fail due to missing tenant:manage
    // permission)
    Response deleteResponse =
        given()
            .header("Authorization", "Bearer " + limitedAdminToken)
    ...
    assertStatusCode(getResponse, 200);
    getResponse.then().body("name", equalTo(tenantName));
