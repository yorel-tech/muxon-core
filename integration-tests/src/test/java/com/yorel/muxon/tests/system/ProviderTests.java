...
/**
 * Integration tests for provider API: HATEOAS links, libvirt provider with localhost, and
...
public class ProviderTests extends BaseIntegrationTest {
  ...
  public void testGetProviderReturnsLinks() {
    ...
    Map<String, String> credentials =
        Map.of("sshPrivateKey", ProviderTestUtil.MINIMAL_SSH_PRIVATE_KEY);
    Response createResponse =
        ProviderTestUtil.createProvider(
            name, ProviderType.LIBVIRT, "ssh://root@localhost:22", credentials);
  ...
  public void testListProvidersReturnsLinksInItems() {
    ...
    Response createResponse =
        ProviderTestUtil.createProvider(
            name, ProviderType.LIBVIRT, "libvirt://localhost", credentials);
    assertStatusCode(createResponse, 201);
    String providerId = createResponse.jsonPath().getString("id");
    ...
    listResponse
        .then()
        .body(
            "items.find { it.id == '%s' }._links".formatted(providerId),
            anyOf(notNullValue(), nullValue()));
    listResponse
        .then()
        .body(
            "items.find { it.id == '%s' }.links".formatted(providerId),
            anyOf(notNullValue(), nullValue()));
  ...
  public void testCreateLibvirtProviderWithSshLocalhost() {
    ...
    response
        .then()
        .body("status", anyOf(equalTo("ACTIVE"), equalTo("CONNECTING"), equalTo("ERROR")));
  ...
  public void testValidationInvalidLibvirtEndpoint() {
    ...
    Response response =
        ProviderTestUtil.createProvider(
            name, ProviderType.LIBVIRT, "https://example.com", credentials);
  ...
  public void testValidationMissingCredentials() {
    String name = ProviderTestUtil.generateUniqueProviderName();
    Response response =
        ProviderTestUtil.createProvider(name, ProviderType.LIBVIRT, "libvirt://localhost", null);
    assertStatusCode(response, 500);
    response.then().body("code", equalTo("INTERNAL_ERROR"));
  }
  ...
  public void testValidationLibvirtMissingSshPrivateKey() {
    ...
    Response response =
        ProviderTestUtil.createProvider(
            name, ProviderType.LIBVIRT, "libvirt://localhost", Collections.emptyMap());
    assertStatusCode(response, 500);
    response.then().body("code", equalTo("INTERNAL_ERROR"));
  }
